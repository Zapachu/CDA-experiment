...
function geneClientBuilder(_a) {
  ...
  var namespace = _a.namespace,
    ...
    paths = _a.paths,
    qiNiu = _a.qiNiu;
  var _d = resolvePaths(basePath, paths),
    entry = _d.entry,
    output = _d.output;
  var buildMode = process.env.BUILD_MODE || "dev",
  ...
  return {
    ...
    devServer: {
      ...
      (_b["/" + share_1.config.rootName] = {
        target:
          "http://" +
          util_1.NetWork.getIp() +
          ":" +
          share_1.config.devPort.server,
        ws: true
      }),
      _b)
    },
    ...
    output: {
      ...
      publicPath:
        buildMode === "publish"
          ? qiNiu.download.jsDomain + "/" + qiNiu.upload.path + "/" + namespace
