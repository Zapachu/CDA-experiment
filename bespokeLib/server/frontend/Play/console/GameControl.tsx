...
export class GameControl extends React.Component<IGameControlProps> {
  ...
  render() {
    const {
      lang,
      props: { historyPush, game, gameState, playerStates, frameEmitter }
    } = this
    if (!gameState) {
      return null
    }
      ...
      <section className={style.gameControl}>
        <div className={style.headBar}>
          ...
          <div className={style.btnGroup}>
            ...
            <Button
              {...btnProps}
              icon={ButtonProps.Icon.parameter}
        ...
        <div className={style.statusSwitcher}>
          <div className={style.switcherWrapper}>
            {(this.gameStatusMachine[gameState.status] || []).map(
              ({ status, label, color = ButtonProps.Color.blue, width = ButtonProps.Width.medium }) => (
                <Button
                  key={label}
                  {...{
...
class SimulatePlayer extends React.Component<{ gameId: string }, ISimulatePlayerWrapperState> {
  ...
  async addSimulatePlayer() {
    const {
      MAX_SIZE,
      props: { gameId },
      state: { simulatePlayers, simulateName, simulateNameSeq }
    } = this
    ...
    return (
      <section className={style.simulatePlayer}>
        <div className={style.header}>
          <label>
            {lang.SimulatePlayer}({simulatePlayers.length}/{this.MAX_SIZE})
          </label>
          <div className={style.addWrapper}>
            <input
              value={simulateName}
              placeholder={this.NAMES[simulateNameSeq]}
