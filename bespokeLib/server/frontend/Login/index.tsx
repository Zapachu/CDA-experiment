      ...
      <section className={style.Login}>
        ...
        <section className={style.inputPanel}>
          ...
          <div className={style.verifyCodeWrapper}>
            ...
            />
            {this.state.counter ? (
              <button className={`${style.verifyCodeBtn} ${style.disabled}`}>{this.state.counter}s</button>
            ) : (
              <button className={style.verifyCodeBtn} onClick={() => this.getVerifyCode()}>
                {lang.getVerifyCode}
              </button>
            )}
          </div>
          <div className={style.loginBtnWrapper}>
            <button className={style.loginBtn} onClick={() => this.login()}>
              {lang.login}
            </button>
          </div>
        </section>
...
class MobileNumberInput extends React.Component<IMobileCodeInputProps, IMobileCodeInputState> {
  ...
  render(): React.ReactNode {
    ...
    return (
      <section className={style.mobileNumberInput}>
        <div
          className={style.nationCode}
        ...
        {this.state.showNationSelector && (
          <ul className={style.nationCodeSelector}>
            {Object.entries(NationCode).map(([label, code]: [string, NationCode]) =>
              isNaN(Number(code)) ? null : (
                <li
                  key={label}
                  onClick={() => {
