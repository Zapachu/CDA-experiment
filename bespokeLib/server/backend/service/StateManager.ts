import { Actor, IActor, SyncStrategy, TGameState, TPlayerState } from '@bespoke/share'
import { GameStateSynchronizer, PlayerStateSynchronizer, StateSynchronizer } from './StateSynchronizer'
import { BaseLogic } from './BaseLogic'
import { GameDAO } from './GameDAO'

export class StateManager<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams> {
  private gameStateManager: GameStateSynchronizer<
    ICreateParams,
    IGameState,
    IPlayerState,
    MoveType,
    ...
    IPushParams
  >
  private playerStateManagers: Array<
    PlayerStateSynchronizer<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams>
  > = []
  ...
  constructor(
    ...
    private logic: BaseLogic<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams>
  ) {
  ...
  async getPlayerStates(): Promise<{ [token: string]: TPlayerState<IPlayerState> }> {
    ...
    tokens.forEach(async token => await this.getPlayerManager({ token, type: Actor.player }))
