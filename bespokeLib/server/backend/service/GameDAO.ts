...
export class GameDAO {
  ...
  static saveGameState(gameId: string, gameState: TGameState<any>) {
    elfSetting.inProductEnv &&
      redisClient.set(RedisKey.gameState(gameId), JSON.stringify(gameState)).catch(reason => Log.e(reason))
  }

  static savePlayerState(gameId: string, token: string, playerState: TPlayerState<any>) {
    elfSetting.inProductEnv &&
      redisClient.set(RedisKey.playerState(gameId, token), JSON.stringify(playerState)).catch(reason => Log.e(reason))
  }
