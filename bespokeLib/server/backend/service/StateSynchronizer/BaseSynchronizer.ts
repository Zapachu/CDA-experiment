...
export class PlayerStateSynchronizer<
  ICreateParams,
  IGameState,
  IPlayerState,
  MoveType,
...
> {
  ...
  constructor(
    public actor: IActor,
    protected controller: BaseLogic<
      ICreateParams,
      IGameState,
      IPlayerState,
      MoveType,
  ...
  async getState(): Promise<TPlayerState<IPlayerState>> {
    if (!this._state) {
      this._state =
        (await GameDAO.queryPlayerState<IPlayerState>(this.controller.game.id, this.actor.token)) ||
