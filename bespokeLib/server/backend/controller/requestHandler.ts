...
Object.assign(window, {
    ...
    PRODUCT_ENV:${elfSetting.inProductEnv}
        ...
        .replace(/static/g, `${Setting.namespace}/static`) +
  ...
  static async handleLogin(req, res: Response, next) {
    const {
      body: { nationCode, mobile, verifyCode },
      session: { returnToUrl }
    } = req
...
export class GameCtrl {
  ...
  static async getSimulatePlayers(req: Request, res) {
    ...
    const simulatePlayers = (await SimulatePlayerModel.find({ gameId })).map(({ gameId, token, name }) => ({
      gameId,
      token,
      name
    }))
