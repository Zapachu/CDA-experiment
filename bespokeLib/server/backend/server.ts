...
export class Server {
  ...
  static start(
    namespace: string,
    Logic: new (...args) => AnyLogic,
    staticPath: string,
    bespokeRouter: Express.Router = Express.Router(),
    startOption: IStartOption = {}
  ) {
    ...
    this.bindServerListener(server, () => {
      Log.i(
        `Running at：http://${Setting.ip}:${elfSetting.bespokeHmr ? config.devPort.client : Setting.port}/${
          config.rootName
        }/${Setting.namespace}`
      )
  ...
  private static initExpress(bespokeRouter: Express.Router): Express.Express {
    ...
    express.use(
      `/${config.rootName}/${Setting.namespace}/static`,
      Express.static(Setting.staticPath, { maxAge: '10d' })
    )
    express.use(
      `/${config.rootName}/${Setting.namespace}/static`,
  ...
  private static withLinker() {
    RedisCall.handle<Linker.Create.IReq, Linker.Create.IRes>(
      Linker.Create.name(Setting.namespace),
      async ({ elfGameId, owner, params }) => {
        const { id } = await GameModel.create({
          namespace: Setting.namespace,
          title: `WithLinker:${Setting.namespace}`,
          elfGameId,
          owner,
          params
        })
