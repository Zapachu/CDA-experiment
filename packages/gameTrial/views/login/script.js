function LoginInit(_csrf, __rootname, lanCtrlObj, cb) {
  //登录组件引用了模态框组件Modal   cb为登录成功后的回调函数
  // if (!_csrf || !lanCtrlObj) {
  ...
  var loginRequest = function() {
    ...
    wechatScanSigninPending = function() {
      return $.post('/wxopen/logincheck', { _csrf: _csrf })
        .done(function(result) {
          if (loginType !== 'wx') return
  ...
  function renderErrorMsg(sibling, msg) {
    ...
    $(sibling).after('<div class="errMsg">' + msg + '</div>')
  }
