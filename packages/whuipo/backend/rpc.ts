...
export async function sendBackData(
  iLabXUserName: string,
  childProjectTitle: string
) {
  //TODO 字段待完善
  const score = ~~(Math.random() * 30 + 70),
    timeUsed = Math.random() * 10 + 2;
  const data = {
    username: iLabXUserName,
    projectTitle: "金融市场与算法交易虚拟仿真实验教学项目",
    childProjectTitle,
    status: 1,
    score,
  ...
  try {
    ...
    const response = await request({
      url: `${
        setting.iLabXGateWay
      }/project/log/upload?xjwt=${encodeURIComponent(token)}`
    });
    Log.d(response);
  } catch (e) {
...
export function runRPC() {
  RedisCall.handle<Trial.Done.IReq, Trial.Done.IRes>(
    ...
    async ({ userId, namespace }) => {
      ...
      const phaseServerUsage = await redisClient.get(
        RedisTools.phaseServerUsageKey(phase)
      );
