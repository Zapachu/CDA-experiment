...
import {getGameLogic} from '../manager/logicManager'
...
export const EventHandler = {
    ...
    [baseEnum.SocketEvent.disconnect]: async ({game, actor}: IConnection) => {
        const {stateManager} = await getGameLogic(game.namespace).getGameController(game.id)
        if (actor.type === baseEnum.Actor.owner) {
    ...
    [baseEnum.SocketEvent.move]: async ({actor, game}: IConnection, type: string, params: {}, cb?: IMoveCallback) => {
        const controller = await getGameLogic(game.namespace).getGameController(game.id)
