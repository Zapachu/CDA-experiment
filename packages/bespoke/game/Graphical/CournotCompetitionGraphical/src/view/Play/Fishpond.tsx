...
const FishPool = ({playerStatus}) => {
    ...
    return <>
        <g transform={`translate(${span(3.9)},${span(6)})`}>
            ...
            <Fish active={true} playerStatus={playerStatus} origin={[.3, .8]}/>
            <Fish playerStatus={playerStatus} origin={[1.1, 1.1]}/>
            <Fish active={true} playerStatus={playerStatus} origin={[1.9, .8]}/>
        </g>
    </>
}
...
function Fish({active=false, origin, playerStatus}) {
    ...
    if(!active || playerStatus === PlayerStatus.prepared) {
        return <image {...{href: fishSrc, width: span(.7), x: span(origin[0]), y: span(origin[1])}}/>
    }
    const {x, y, width} = useSpring({
        x: -1.6, y: 2, width: 0,
        from: {x: origin[0], y: origin[1], width: .7},
        config: {
            duration: 800
        }
    })
