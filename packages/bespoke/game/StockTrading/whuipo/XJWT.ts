...
class XJWT {
  ...
  static encode(type: Type, payload?: object): string {
    try {
      ...
      const headerStr = encodeHeaderToBase64(type);
      const payloadStr = encodePayloadToBase64(type, payload);
      return sign(headerStr, payloadStr);
    } catch (e) {
      console.error(e);
      return null;
    }
...
function sign(header: string, payload: string): string {
  const signature = _sha256(header + "." + payload);
  return header + "." + payload + "." + signature;
}

function encodeHeaderToBase64(type: Type): string {
  ...
  const headerBuf = Buffer.concat([expiryBuf, typeBuf, issuerIdBuf], bufLength);
  return headerBuf.toString("base64");
}

function encodePayloadToBase64(type: Type, payload: object): string {
  ...
  const payloadBuf = Buffer.concat([randomBuf, bodyBuf, paddingBuf], bufLength);
...
function repairToken(token: string): string {
  ...
  // console.log(repairedToken);
  return repairedToken;
}

function checkTimestamp(header: string): boolean {
...
function extractType(header: string): Type {
  const buf = Buffer.from(header, "base64");
  const type = buf.readInt8(HEADER_TYPE_OFFSET);
  const TYPES = { [Type.RESERVED]: true, [Type.JSON]: true, [Type.SYS]: true };
...
function checkSignature(
  ...
  const _sig = _sha256(header + "." + payload);
...
// only node 10+ supported, while negative number not supported yet
