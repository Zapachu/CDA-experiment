...
import {Button, ButtonProps, MaskLoading, Core, Lang, Toast, Label, Input, Request} from 'bespoke-client-util'
import {FetchRoute, MoveType, PushType, Stage, namespace} from '../../config'
...
export class Play extends Core.Play<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams, IPlayState> {
    ...
    componentDidMount() {
      const {playerState:{actor}, frameEmitter} = this.props;
      frameEmitter.emit(MoveType.initPosition);
      Request.get(namespace, FetchRoute.getUserMobile, {token: actor.token, actorType: actor.type})
    }
