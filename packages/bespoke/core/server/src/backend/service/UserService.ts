...
export default class UserService {
    ...
    static async sendVerifyCode(nationCode: baseEnum.NationCode, mobile: string): Promise<{
        ...
        Log.d(verifyCode)
        const sendSuccess = !elfSetting.inProductEnv || await QCloudSMS.singleSenderWithParam(nationCode, mobile, elfSetting.qCloudSMS.templateId.verifyCode, [verifyCode])
        if (sendSuccess) {
