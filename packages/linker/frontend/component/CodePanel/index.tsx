...
export class CodePanel extends React.Component<ICodePanelProps, ICodePanelState> {
  ...
  render() {
    ...
    return (
      <section className={style.codePanel}>
        <div className={style.password}>
          {code.map((c, i) => (
            <span key={i} className={i >= active ? style.disabledClick : ''} onClick={e => this.handleItemClick(i, e)}>
              {focus === i ? (
                <input
                  key={'input' + i}
                  ref={input => input && input.focus()}
                  className={style.focusInput}
                  value={inputValue}
                />
              ) : (
                c
          ...
          {Object.entries(Keyboard).map(([row, keys]) => (
            <div key={row}>
              {Object.values(keys).map(v => (
                <span key={v} className={style.inputKey} onClick={() => this.setCode(v)}>
                  {v}
                </span>
              ))}
            </div>
