import settings from '../../core/server/config/settings'
const { otreePhaseServerPrefix } = settings
...
async function request(url): Promise<any> {
    ...
    const res = await fetch(`${otreePhaseServerPrefix}${url}`, option)
    if (res.ok) {
        return res.json()
    }
}
...
function buildPath(pathFragment, params) {
    const path = `/${otreePhaseServerPrefix}${pathFragment}`
    if (!params) {
        return path
    }
