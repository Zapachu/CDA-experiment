...
└───lib
    └───core
        ...
        └───server
            └───config    settings 文件
            └───models    model 文件
            └───otree     otree服务器，包括rpc、反向代理
            └───quatircs  TODO
            └───rpc       otree rpc,TODO 后续扩展为多个rpc服务或整合多个 第三方phase
            └───view      phase 组件依赖的视图文件
    └───phase
        └───otree         第三方otree phase     [Process: 90%]
        └───quatrics      第三方quatrics phase  [Process: 10%]
```
