...
> {
  initGameState(): TGameState<IGameState> {
    ...
    gameState.rounds = Array(ROUND)
      .fill(null)
      .map<IGameRoundState>(() => ({ time: 0, shouts: [], trades: [] }))
  ...
  async startPrepareCountDown() {
    ...
    const timer = global.setInterval(async () => {
      if (++gameState.prepareTime === PREPARE_TIME) {
        ...
        Array(PLAYER_NUM - gameState.playerIndex)
          .fill(null)
          .forEach(async (_, i) => {
  ...
  async playerMoveReducer(actor: IActor, type: string, params: IMoveParams, cb: IMoveCallback): Promise<void> {
    ...
    switch (type) {
      case MoveType.getIndex: {
        ...
        playerState.privatePrices = Array(ROUND)
          .fill(null)
          .map(() => ~~(Math.random() * 20 * (playerState.role === Role.seller ? -1 : 1) + 70))
