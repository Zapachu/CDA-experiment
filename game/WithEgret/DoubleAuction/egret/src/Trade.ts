...
class Trade extends Scene<TradeState> {
  ...
  protected childrenCreated(): void {
    ...
    this.priceInput.addEventListener(
      egret.Event.FOCUS_OUT,
      ({ target: { text } }: egret.Event) => {
        const price = Number(text)
        const {
            playerState: { role, privatePrices },
            gameState: { roundIndex }
          } = IO,
          privatePrice = privatePrices[roundIndex],
          priceInvalid =
            isNaN(price) ||
            (role === Role.seller && price < privatePrice) ||
            (role === Role.buyer && price > privatePrice)
        if (priceInvalid) {
  ...
  render() {
    ...
    this.tip.text = `您的角色为 : ${playerState.role === Role.seller ? '卖家' : '买家'}${
      myShout && myShout.traded ? ',交易成功' : ''
    }`
    ...
    this.tradeHistory.dataProvider = new eui.ArrayCollection(
      rounds.map(({ shouts, trades }, i) => {
        const myTrade = trades.find(
          ({ reqIndex, resIndex }) => reqIndex === playerState.index || resIndex === playerState.index
        )
        if (!myTrade) {
          return { round: i + 1 }
        }
