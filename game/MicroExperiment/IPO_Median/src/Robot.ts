...
function genPriceAndNum(min: number, max: number, startMoney: number): { price: number; bidNum: number } {
  const price = formatDigits(genRandomInt(min * 100, max * 100) / 100)
  const maxNum = Math.floor(startMoney / price)
  const num = genRandomInt(BuyNumberRange.robotMin / 100, Math.min(BuyNumberRange.robotMax, maxNum) / 100) * 100
  ...
  async init(): Promise<this> {
    ...
    this.frameEmitter.on(PushType.startRound, () => {
      setTimeout(() => {
        ...
        const { startMoney, privateValue } = this.playerRoundState,
          { minPrice } = this.gameRoundState
