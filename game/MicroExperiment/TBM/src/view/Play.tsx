...
> {
          ...
          <li style={{ marginTop: 12 }}>
            <a className={style.countLimit}>
              ...
              <span className={style.priceHighlight}>
                {!isNaN(parseInt(price)) && parseInt(price) !== 0 ? Math.floor(startingPrice / Number(price)) : ''}
              </span>{' '}
              股
            </a>
          </li>
          ...
          <li style={{ marginTop: 12 }}>
            <div className={style.feeInfo}>
              <a className={style.feeNumber}>
                总花费
                <span className={style.priceHighlight}>
                  {count !== '' && price !== '' ? Number(count) * Number(price) : ''}
                </span>
              </a>
  ...
  dynamicBtnView = () => {
    ...
    return <Button label={this.dynamicTip()} onClick={this.shout} style={{ marginTop: 36 }} />
  }
  ...
  renderResult = () => {
    ...
    const listData = [
      ...
      { label: '你的总收益', value: profit.toFixed(2) },
      {
        ...
        value:
          role === Role.Buyer
            ? `资金${(startingPrice - strikePrice * actualNum).toFixed(2)}元; 股票${actualNum}股`
            : `资金${(strikePrice * actualNum).toFixed(2)}元; 股票${startingQuota - actualNum}股`
      }
    ]
    return (
      <>
        ...
        <div className={style.tradeBtn}>
          <Button onClick={this.showTBMRule} color={Button.Color.Blue} label={`集合竞价知识扩展`} />
        </div>
        <ul>
  ...
  renderPlay = () => {
    ...
    return (
      <section className={style.playContent}>
        ...
        <table className={style.stockInfo}>
          <tbody>
            <tr>
              ...
              <td>主承销商</td>
              <td>
                <span className={style.startDate}>初步询价</span>起始日期
              </td>
              <td>初步询价截止日期</td>
            </tr>
            <tr>
        ...
        <p className={style.countDown}>{SHOUT_TIMER - shoutTime} S</p>
        {this.dynamicInfo()}
        <div className={style.tradeBtn}>
          <Button onClick={this.showRule} color={Button.Color.Blue} label={`交易规则回顾`} />
        </div>
      </section>
    )
  }
    ...
    const questions: Array<ITestPageQuestion> = [
      {
        ...
        Answer: () => (
          <text>
            正确答案：B , 初步询价原则上只能有一次报价，因特殊原因需要调整的，应在申购平台上填写具体原因修改。
          </text>
        ),
        answer: [2]
      },
      {
        Content: ({ inputProps }) => (
          <p>
            假设有3个买家：A, B, C和3个卖家：D, E,
            F。将3个买家的买入价格按照从高到低排序，卖家的出售价格按照从低到高排序
            <br />
            买家：
            <br />
            B的每份配额买入价格是62元，购买配额数量是8股；
            <br />
            C的每份配额买入价格是58元，购买配额数量是5股；
            <br />
            A的每份配额买入价格是55元，购买配额数量是4股；
            <br />
            卖家：
            <br />
            E的每份配额出售价格是52元，出售配额数量是4股；
            <br />
            D的每份配额出售价格是56元，出售配额数量是6股；
            <br />
            F的每份配额出售价格是63元，出售配额数量是5股；
            <br />
            1）
            最高买入价格的B的买入价格高于最低卖出价格E的卖出价格，E的四股股票全部成交，B可购入4股E的股票，B还有四股股票未购入；
            <br />
            2） 最高买入价格的B的买入价格高于第二低卖出价格D的卖出价格，B还未成交的四股股票成交，此时D还剩2股；
            <br />
            3） 第二高买入价格的C的买入价格高于第二低卖出价格D的卖出价格，D剩下的2股股票成交，C还有3股未成交；
            <br />
            4） 第三高买入价格的A的买入价格低于D的卖出价格，无法成交；
            <br />
            故成交价格为（58+56）/2=
            <AntInput {...inputProps()} />
            ，成交10股
      ...
      },
      {
        Content: ({ inputProps }) => (
          <p>
            假设有还有另外2个卖家：B, C和3个卖家：D, E,
            F。将3个买家的买入价格按照从高到低排序，卖家的出售价格按照从低到高排序
            <br />
            买家：
            <br />
            B的每份配额买入价格是48元，购买配额数量是7股；
            <br />
            C的每份配额买入价格是44元，购买配额数量是4股；
            <br />
            您的每份配额买入价格是39元，购买配额数量是5股；
            <br />
            卖家：
            <br />
            E的每份配额出售价格是40元，出售配额数量是4股；
            <br />
            D的每份配额出售价格是42元，出售配额数量是6股；
            <br />
            F的每份配额出售价格是46元，出售配额数量是5股；
            <br />
            故成交价格为（44+42）/2=
            <AntInput {...inputProps()} />
            ，B买入
        ...
        Answer: () => (
          <text>
            <br />
            1）最高买入价格的B的买入价格高于最低卖出价格E的卖出价格，E的四股股票全部成交，B可购入4股E的股票，B还有3股股票未购入；
            <br />
            2）最高买入价格的B的买入价格高于第二低卖出价格D的卖出价格，B还未成交的3股股票成交，此时D还剩3股；
            <br />
            3）第二高买入价格的C的买入价格高于第二低卖出价格D的卖出价格，D剩下的3股股票成交，C还有1股未成交；
            <br />
            4）第三高买入价格您的买入价格低于D的卖出价格，无法成交；
            <br />
            成交价格为（44+42）/2= 43 ，B买入 7 股，C买入 3 股，您买入 0 股，E卖出 4 股，D卖出 6 股，F卖出 0 股
          </text>
        ),
      ...
      },
      {
        Content: ({ inputProps }) => (
          <p>
            假设有还有另外2个卖家：B, C和3个卖家：和4个卖家：D, E,
            F,G。将3个买家的买入价格按照从高到低排序，卖家的出售价格按照从低到高排序
            <br />
            买家：
            <br />
            B的每份配额买入价格是48元，购买配额数量是7股；
            <br />
            C的每份配额买入价格是45元，购买配额数量是4股；
            <br />
            您的每份配额买入价格是39元，购买配额数量是5股；
            <br />
            卖家：
            <br />
            G的每份配额出售价格是40元，出售配额数量是4股；
            <br />
            D的每份配额出售价格是42元，出售配额数量是4股；
            <br />
            F的每份配额出售价格是43元，出售配额数量是5股；
            <br />
            E的每份配额出售价格是46元，出售配额数量是5股；
            <br />
            成交价格为（45+43）/2=
            <AntInput {...inputProps()} />
            ，B买入
        ...
        Answer: () => (
          <text>
            <br />
            1）最高买入价格的B的买入价格高于最低卖出价格G的卖出价格，G的四股股票全部成交，B可购入4股E的股票，B还有3股股票未购入；
            <br />
            2）最高买入价格的B的买入价格高于第二低卖出价格D的卖出价格，B还未成交的3股股票成交，此时D还剩1股；
            <br />
            3）第二高买入价格的C的买入价格高于第二低卖出价格D的卖出价格，D剩下的1股股票成交，C还有3股未成交；
            <br />
            4）第二高买入价格的C的买入价格高于第三低卖出价格F的卖出价格，C剩下的3股股票全部成交，F还剩2股没有成交。
            <br />
            4）第三高买入价格您的买入价格低于F的卖出价格，无法成交；
            <br />
            故成交价格为（45+43）/2=44，B买入7股，C买入4股，您买入0股，G卖出4股，D卖出4股，F卖出3股，E卖出0股。
          </text>
        ),
        ...
        <Modal
          ...
          children={
            <div className={style.modalContent}>
              ...
              <Button style={{ marginTop: '30px' }} label={'关闭'} color={Button.Color.Blue} onClick={this.showRule} />
            </div>
          }
        />
        <Modal
    ...
    steps: Array<Step> = [
      {
        ...
        content: (
          <section className={style.guideContent}>
            <p>
              初步询价是指发行人及其保荐机构向询价对象进行询价，并根据询价对象的报价结果确定发行价格区间及相应的市盈率区间。通俗点讲，企业和主承销商向询价对象（一般是指在中国证监会备案的基金管理公司、投资机构、证券公司等）推介和发出询价函，以反馈回来的有效报价上下限确定的区间为初步询价区间。这一步做完后，就应该在证监会指定的网站上披露初步询价结果（即初步询价公告）。因特殊原因需要调整初步询价的，应在申购平台上填写具体原因修改。
            </p>
          </section>
        ),
        ...stepProps()
      },
      {
        ...
        content: (
          <section className={style.guideContent}>
            <p>集合竞价时，系统将撮合所有委托单，筛选出满足最大量的可成交价格，确定为正式的开盘价。</p>
          </section>
        ),
        ...stepProps()
      },
