  ...
  return (
    <section className={style.playContent}>
      ...
      <div className={style.tips}>
        <label>最高拟购买价格</label>
        <em>{maxShout}</em>元,若在
        <em className={style.countDown}>{gameRoundState.timer}</em>
        秒内没有更高报价，则此资产以现在的最高拟购买价格成交
        <p className={style.auction}>
          <label>市场信息</label>该资产的
          <span className={style.startPrice}>
            起拍价格为<em>{gameRoundState.startPrice}</em>元
          </span>
        </p>
        <label>私人信息</label>您的公司对该资产的估价值为
        <em>{playerRoundState.privatePrice}</em>元
      </div>
  ...
  const questions: Array<ITestPageQuestion> = [
    {
      Content: ({ inputProps }) => (
        <div>
          假设某资产现在进行拍卖，现最高的竞购价格为100元，经拍卖人三次提示而无人加价时，拍卖人击槌表示该资产以
          <AntInput {...inputProps()} />
          元成交
        </div>
      ),
    ...
    },
    {
      Content: ({ inputProps }) => (
        <div>
          <p>
            以公开竞价的方法，组织有意受让的人到场竞价报价，最后以“价高者得“的原则确定土地使用权受让人的一种土地出让方式为
          </p>
...
function Result({
  ...
  const tradePrice = shouts[tradeShoutIndex],
    profit =
      playerState.index === tradeShoutIndex ? (playerState.rounds[round].privatePrice - tradePrice).toFixed(2) : 0
  return (
    <section className={style.result}>
      <p className={style.resultInfo}>
        该资产以<em>{tradePrice}</em>元成交，由
        {playerState.index === tradeShoutIndex ? '您' : '其它卖家'}
        竞购得到，您的收益为<em>{profit}</em>元
      </p>
      {round === ROUNDS - 1 ? (
        <>
          <Line
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <Button
              ...
              onClick={() => exit(true)}
            />
            <Button label={'返回交易大厅'} onClick={() => exit()} />
          </div>
        </>
      ) : (
