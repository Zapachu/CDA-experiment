...
export default class Robot extends BaseRobot<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams> {
    ...
    async zipInit(): Promise<this> {
        ...
        setTimeout(() => {
            const u = (this.zipRole === ROLE.Seller ? 1 : -1) * (.05 + .3 * Math.random()),
    ...
    zipRespondNewOrder(newOrderId: number) {
        const {orderDict, zipRole, zipFreeField: {calcPrice}} = this,
            newOrder = orderDict[newOrderId];
        const newOrderRole = newOrder.role;
        if (newOrderRole === zipRole &&
    ...
    zipRespondNewTrade(resOrderId: number) {
        ...
        if ((zipRole === ROLE.Buyer && this.zipFreeField.calcPrice >= tradePrice) ||
