...
> {
  initGameState(): TGameState<IGameState> {
    ...
    gameState.periods = Array(PERIOD)
      .fill(null)
      .map(
        () =>
          ({
  ...
  async shoutNewOrder(periodIndex: number, order: IOrder): Promise<void> {
    ...
    if (tradeSuccess) {
      ...
      playerStates.forEach(playerState => {
        const o =
          playerState.playerIndex === order.playerIndex
            ? order
            : playerState.playerIndex === pairOrder.playerIndex
            ? pairOrder
            : null
        if (!o) {
          return null
        }
        this.push(playerState.actor, PushType.tradeSuccess, {
  ...
  async getActivePlayerStates(playerIndex?: number): Promise<TPlayerState<IPlayerState>[]> {
    ...
    return Object.values(playerStates).filter(s =>
      playerIndex ? s.playerIndex === playerIndex : s.playerIndex !== undefined
    )
  }
  ...
  protected async playerMoveReducer(
    ...
    switch (type) {
      ...
      case MoveType.getIndex: {
        ...
        switch (true) {
          case actor.type === Actor.serverRobot &&
            playerStates.every(({ identity }) => identity != Identity.moneyGuarantor):
            playerState.identity = Identity.moneyGuarantor
            playerState.count = 0
            playerState.money = gameState.initialAsset.money * playerLimit
            break
          case actor.type === Actor.serverRobot &&
            playerStates.every(({ identity }) => identity != Identity.stockGuarantor):
        ...
        const timer = global.setInterval(async () => {
          ...
          switch (periodCountDown) {
            case ~~(prepareTime / 2): {
              if (periodIndex === 0) {
                Array(2 + playerLimit - gameState.playerIndex)
                  .fill(null)
                  .forEach(async (_, i) => await this.startRobot(`$Robot_${i}`))
              }
              break
            }
            case prepareTime: {
            ...
            case prepareTime + tradeTime: {
              ...
              playerStates.forEach(async playerState => {
                const asset = gamePeriodState.balancePrice * playerState.count + playerState.money,
                  guaranteeAsset =
                    gamePeriodState.balancePrice * playerState.guaranteeCount + playerState.guaranteeMoney
            ...
            case 0: {
              ...
              const [min, max] = PrivatePriceRegion[gameState.type][periodIndex],
                ...
                privatePrices = playerStates.map(() => random(min, max)).sort((m, n) => (m - n) * periodTrend)
              playerStates
                .sort((p1, p2) => p1.count - p2.count)
