...
function Border({
  background = `radial-gradient(at 50% 0%, #67e968 1rem, transparent 70%)`,
  borderRadius = '1rem',
  children,
  style
}: {
            ...
            <React.Fragment key={i}>
              <line
                {...{
                  ...
                  strokeWidth: i === 0 ? 1 : i % 5 === 0 ? 0.3 : 0.1,
                  x1: transX(i),
                  y1: transY(minY),
                  x2: transX(i),
                  y2: transY(maxY)
              ...
              {i && i % 5 === 0 ? (
                <text
                  {...{
                    fontSize,
                    fill: color.number,
                    x: transX(i - (0.5 * fontSize) / cellSize),
              ...
              {i && i % 5 === 0 ? (
                <text
                  {...{
                    ...
                    y: transY(minY + (i - (0.5 * fontSize) / cellSize) * cellSize)
                  }}
                >
                  {minY + i * cellSize}
                </text>
...
function Result({
  ...
  const lang = Lang.extractLang({
    ...
    close: ['关闭', 'Close'],
    tipsContent: [
      '连续竞价是指买卖股票时，由电脑交易系统连续撮合买卖委托，产生出成交价的一种交易机制。沪深两市上午连续竞价时间一致：09：30~11：30。但是下午就略有不同了，沪市：13：00~15：00，深市13：00~14：57。\n' +
...
function _Play({
  ...
  const lang = Lang.extractLang({
    ...
    invalidBuyPrice: [
      '订单价格需高于市场当前最高买价',
      'Order price must be lower than the highest buy price in the market'
    ],
    invalidSellPrice: [
      '订单价格需低于市场当前最低卖价',
      'Order price must be higher than the lowest buy price in the market'
    ],
    ...
    closeOutWarning: [
      '资产低于担保金额130%将被平仓',
      'Your count will be closed out when assets is below 130% of guarantee money'
    ],
    closedOut: [
      '资产低于担保金额130%，已被平仓',
      'Your count has been closed out since assets is below 130% of guarantee money'
    ],
  ...
  if (gamePeriodState.stage === PeriodStage.result) {
    return (
      <section className={style.resultWrapper}>
        ...
        />
        {renderTradePanel()}
        {gameState.periodIndex === PERIOD - 1 ? (
          <section className={style.switchBtns}>
            <Button label={lang.onceMore} onClick={() => exitGame(true)} />
            &nbsp;
            <Button label={lang.nextPhase} onClick={() => exitGame()} />
          </section>
        ) : (
          <p className={style.toNextPeriod}>
            {lang.toNextPeriod1}
    ...
    const questions: Array<ITestPageQuestion> = allowLeverage
      ...
      : [
          ...
          },
          {
            Content: ({ inputProps }) => (
              <div>
                挂在买家订单上面的最高买价是11，挂在卖家订单上面的最低卖价是12。在这个时候同时出现了一个买入价格为13元的买价订单，则成交价格为
                <AntInput {...inputProps()} />
              </div>
            ),
          ...
          },
          {
            Content: ({ inputProps }) => (
              <div>
                挂在买家订单上面的最高买价是11，挂在卖家订单上面的最低卖价是12。在这个时候同时出现了一个卖出价格为10元的卖家订单，则成交价格为
                <AntInput {...inputProps()} />
              </div>
            ),
  ...
  const guaranteeMoneyLimit = nonNegative(playerState.money - playerState.guaranteeMoney),
    guaranteeCountLimit = nonNegative(
      ~~(playerState.money / gamePeriodState.closingPrice) + playerState.count - playerState.guaranteeCount
    )
  return (
    <section className={style.trading}>
      {playerState.status === PlayerStatus.guide ? (
        <Guide allowLeverage={allowLeverage} done={() => frameEmitter.emit(MoveType.guideDone)} />
      ) : null}
  ...
  function submitOrder(role: ROLE, guarantee?: boolean) {
    ...
    if (guarantee) {
      countInvalid =
        count <= 0 ||
        (role === ROLE.Buyer && _price * +count > guaranteeMoneyLimit) ||
        (role === ROLE.Seller && count > guaranteeCountLimit)
    } else {
      countInvalid =
        count <= 0 ||
        (role === ROLE.Buyer && _price * +count > playerState.money) ||
      ...
      inputFragment = (
        <>
          <label className={style.subLabel}>
            {lang.priceCountTips}
            {total && !isNaN(total) ? `${price}*${count}=${total}` : ''}
          </label>
          <br />
          {inputPrice}
          <br />
    ...
    return (
      <div className={style.orderPanel}>
        ...
        <section className={style.orderBook}>
          {renderOrderList(
            buyOrderIds.map(id => orderDict[id]),
            ROLE.Buyer
          )}
          {renderOrderList(
            sellOrderIds.map(id => orderDict[id]),
            ROLE.Seller
          )}
        </section>
        {
          <section className={style.orderSubmission}>
            {gamePeriodState.stage === PeriodStage.trading || playerState.status === PlayerStatus.guide ? (
              <section className={style.newOrder}>
                <label className={style.subLabel}>
                  {lang.timeLeft(gameState.periodIndex + 1, countDown < prepareTime ? '' : timeLeft > 0 ? timeLeft : 0)}
                </label>
                <label className={style.label}>
                  {lang.valuation}
                  <em>{privatePrice}</em>
                </label>
                <Tabs labels={[lang.buy, lang.sell]} activeTabIndex={orderTabIndex} switchTab={setOrderTabIndex}>
            ...
            ) : prepareTime > countDown ? (
              <p className={style.marketWillOpen}>
                {lang.marketWillOpen1}
        ...
        <Border background={STYLE.mainPanelBorder} style={{ maxWidth: '24rem', margin: 'auto' }}>
          <div className={style.tradeListWrapper}>
            <table className={style.tradeList}>
              ...
              <tbody>
                {trades
                  .map(({ reqOrderId, resOrderId, count }, i) => {
  ...
  function renderChartPanel() {
    const chart = (
      <div className={style.marketHistory} onClick={() => setShowChartModal(!showChartModal)}>
        <TradeChart
    ...
    return (
      <div className={style.marketHistoryWrapper}>
        ...
        <Border background={STYLE.mainPanelBorder}>
          {chart}
          <Modal visible={showChartModal}>{chart}</Modal>
        </Border>
      </div>
    )
  }
        ...
        <table className={style.orderTable}>
          ...
          <tbody>
            {marketOrders.map(order => {
              ...
              return (
                <tr key={order.id} className={style.orderPrice}>
                  <td>{order.price}</td>
                  ...
                  <td>
                    <a
                      ...
                      onClick={() => frameEmitter.emit(MoveType.cancelOrder)}
                    >
                      ×
                    </a>
                  </td>
  ...
  const stepProps: Partial<Step> = {
    ...
    commonSteps: Array<Step> = [
      ...
      },
      {
        ...
        content: (
          <GuideContent>
            您可以在这里查看已经提交的买家报价和卖家报价。买单和卖单分别排队，买单以价格从高到低排列，卖单以价格从低到高排列。同价的，按进入系统的先后排列。系统按照顺序将排在前面的买单和卖单配对成交，即按“价格优先，同等价格下时间优先”的顺序依次成交在，直到不能成交为止，未成交的委托排队等待成交。
          </GuideContent>
        ),
        styles: {
          tooltip: {
      ...
      },
      {
        ...
        content: (
          <GuideContent>您可以在这里查看当前的交易期数和剩余交易时间，您需在规定的时间内完成交易。</GuideContent>
        ),
        ...stepProps
      },
      {
        ...
        content: (
          <GuideContent>
            您可以在这里提交您的卖单和卖单。买入申报价高于即时揭示最低卖价，以最低申报卖价成交；卖出申报价低于最高买入价，以最高买入价成交。两个委托如果不能全部成交，剩余的继续留在单上，等待下次成交。系统处理原则为价格优先和时间优先两个原则。
          </GuideContent>
        ),
        ...stepProps
      },
          ...
          },
          {
            ...
            content: (
              <GuideContent>
                ...
                <p>
                  1）
                  卖券还款。指通过信用证券账户申报卖券，结算时卖出证券所得资金直接划转至证券公司融资专用资金账户的一种还款方式；
                </p>
                <p>2） 直接还款。指使用信用资金账户中的现金，直接偿还对证券公司融资负债的一种还款方式；</p>
                <p>偿还融券债务的方法有下列两种：</p>
                <p>
                  1）
                  买券还券。指通过信用证券账户申报买券，结算时买入证券直接划转至证券公司融券专用证券账户的一种还券方式；
                </p>
                <p>
                  2）
                  直接还券。指使用信用证券账户中与其负债证券相同的证券申报还券，结算时期证券直接划转至证券公司融券专用证券账户的一种还券方式。
                </p>
              </GuideContent>
            ),
            ...stepProps
