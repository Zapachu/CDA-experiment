    ...
    return (
      <section className={style.create}>
        <div>
          ...
          <Switch onChange={() => setParams({ allowLeverage: !params.allowLeverage })} checked={params.allowLeverage} />
        </div>
        <div>
          <Label label={lang.robotType} />
          <Select
            defaultValue={CBMRobotType[CBMRobotType.zip]}
            style={{ width: 120 }}
            onChange={(key: string) => setParams({ robotType: CBMRobotType[key] })}
          >
            {getEnumKeys(NCreateParams.CBMRobotType).map(key => (
