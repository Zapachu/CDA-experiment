...
type TPlay4OwnerProps = Core.IPlay4OwnerProps<
  ICreateParams,
  IGameState,
  IPlayerState,
  MoveType,
...
function _Play4Owner({ frameEmitter, gameState, playerStates }: TPlay4OwnerProps) {
  ...
  switch (gamePeriodState.stage) {
    case PeriodStage.reading:
      return (
        <section className={style.waitInfo}>
  ...
  function renderTrading() {
    ...
    return (
      <>
        <label className={style.periodInfo}>{`${lang.trading}  ${lang.timeLeft(
          gameState.periodIndex + 1,
          countDown < prepareTime ? '' : timeLeft > 0 ? timeLeft : 0
        )}`}</label>
        <Row>
          <Col xs={22} md={8} offset={1}>
            <Row>
              <Col span={12}>
                ...
                <List dataSource={sellOrderIds} renderItem={id => <List.Item>{orderDict[id].price}</List.Item>} />
              </Col>
              <Col span={12}>
                <label>{lang.buyOrders}</label>
                <List dataSource={buyOrderIds} renderItem={id => <List.Item>{orderDict[id].price}</List.Item>} />
              </Col>
            </Row>
          </Col>
...
export function playerStatesTable(playerStates: { [token: string]: TPlayerState<IPlayerState> }) {
  ...
  return (
    <Table
      dataSource={Object.values(playerStates).map(
        ({ actor, user, count, money, guaranteeCount, guaranteeMoney }: TPlayerState<IPlayerState>, i) => ({
          key: actor.token,
