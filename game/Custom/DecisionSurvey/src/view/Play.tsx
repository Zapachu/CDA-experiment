...
const ANSWER_LEN_TO_DECISION = [DECISION.one, DECISION.two, DECISION.three, DECISION.four, DECISION.five, DECISION.six]
...
> {
  ...
  constructor(props) {
    ...
    const stage = len === 4 ? STAGE.instruction56 : len > 0 ? STAGE.playing : STAGE.beginning
    this.state = {
      answer: undefined,
      card: undefined,
      stage,
  ...
  renderInstruction = (instructions: Array<string | { text: string; style: object }>) => {
  ...
  renderPlaying = () => {
    const { answer, card, gender, age, institute, grade, name, timer } = this.state
    const { playerState, gameState, frameEmitter } = this.props
    const status = this.getPlayerStatus()
    switch (status) {
      case STATUS.playing: {
        const decision = ANSWER_LEN_TO_DECISION[Object.values(playerState.answer).length]
          ...
          <div className={style.stage}>
                ...
                <div key={i} className={style.question}>
                  ...
                  >
                    {options.map(({ label, value }) => {
                      return (
                        <Radio className={style.radio} key={value} value={value}>
                          {label}
                        </Radio>
                      )
                    })}
            ...
            <Button
              ...
              onClick={() => {
                ...
                frameEmitter.emit(MoveType.shout, { answer: [answer, card], decision }, error => {
                  Toast.warn(error)
                })
      ...
      case STATUS.info: {
        return (
          <div className={style.stage}>
            <Row>问卷决策环节到此结束，下面请填写你的个人信息（我们将严格保密）。</Row>
            <div className={style.info}>
              <label>性别</label>
              <Radio.Group onChange={e => this.setState({ gender: e.target.value as GENDER })} value={gender}>
      ...
      case STATUS.random: {
        return (
          <div className={style.stage}>
            ...
            <Row>再从5和6之中随机抽取一个数字（我们将根据被选中方最终抽取的数字决定你们双方在第二部分的最终收益）</Row>
            <Row>
            ...
            {playerState.random56 ? <Row>等待老师结束实验并计算收益...</Row> : null}
          </div>
        )
      }
      // case STATUS.waiting: {
    ...
    switch (stage) {
      case STAGE.beginning: {
        return (
          <div className={style.stage}>
            <Row>问卷说明</Row>
            <Row>欢迎参加本次的决策问卷，你在本问卷中的所有信息都将被严格保密。</Row>
            <Row>本次问卷共包含6个决策问题，1-4为个人决策问题，5-6为涉及他人的决策问题。</Row>
            <Row>
      ...
      case STAGE.instruction14: {
        return (
          <div className={style.stage}>
            ...
            <Row>4组决策问题均为个人决策，每组决策中你需要在七个选项中选择一个。</Row>
            <Row>最终我们将在4组问题中随机选择1组，并根据你在该组中的选择实现你的最终收益。</Row>
            <Row>
              <Button
                disabled={timer > 0}
                onClick={() => {
