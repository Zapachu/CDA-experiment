...
> {
  ...
  protected async teacherMoveReducer(
    ...
    switch (type) {
      case MoveType.dealCard: {
        ...
        const validPlayerStateArray = playerStateArray.filter(ps => !!ps.random56)
  ...
  protected async playerMoveReducer(
    ...
    switch (type) {
      ...
      case MoveType.random: {
        ...
        const decision14 = [DECISION.one, DECISION.two, DECISION.three, DECISION.four]
  ...
  private processProfit(
    ...
    playerStateArray.forEach(ps => {
      this.setPhaseResult(ps.actor.token, {
        ...
        uniKey: ps.mobile ? `手机号${ps.mobile}的收益: ${this.getProfitString(ps.profit)}` : '-'
      })
    })
  }
    ...
    playerStateArray.forEach(ps => {
      const row = [
        ...
        ps.answer[DECISION.one]
          ? `${
              PAGE[DECISION.one].questions[0].options.find(option => option.value === ps.answer[DECISION.one][0]).label
            }; ${ps.answer[DECISION.one][1]}`
          : '',
        ps.answer[DECISION.two]
          ? `${
              PAGE[DECISION.two].questions[0].options.find(option => option.value === ps.answer[DECISION.two][0]).label
            }; ${ps.answer[DECISION.two][1]}`
          : '',
        ps.answer[DECISION.three]
          ? PAGE[DECISION.three].questions[0].options.find(option => option.value === ps.answer[DECISION.three][0])
              .label
          : '',
        ps.answer[DECISION.four]
          ? PAGE[DECISION.four].questions[0].options.find(option => option.value === ps.answer[DECISION.four][0]).label
          : '',
        ps.answer[DECISION.five]
          ? PAGE[DECISION.five].questions[0].options.find(option => option.value === ps.answer[DECISION.five][0]).label
          : '',
        ps.answer[DECISION.six]
          ? PAGE[DECISION.six].questions[0].options.find(option => option.value === ps.answer[DECISION.six][0]).label
          : '',
  ...
  private _calcProfits(
    ...
    playerStateArray.forEach(ps => {
      ...
      if (ps.won56) {
        const profit56 = calc56(ps.answer[decision56]) as Array<Partial<PROFIT>>
        this._addProfit(ps.profit56, profit56[0])
        this._addProfit(ps.profit, profit56[0])
      } else {
        const pair = playerStates[ps.pair]
        const profit56 = calc56(pair.answer[decision56]) as Array<Partial<PROFIT>>
...
const CALC_PROFIT = {
  ...
  [DECISION.two]: (answer: Array<string>, card1?: CARD, card2?: CARD): Partial<PROFIT> => {
    const start = 20
    const invest = +answer[0].split(':')[0]
    const chosenCard = answer[1]
    let profit: number
