...
class GroupPlay4Owner extends Extend.Group.Play4Owner<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams> {
    render(): React.ReactNode {
        const {groupPlayerStates, groupGameState: {contribution}} = this.props;
            ...
            },
            {
                ...
                children: projectConfigs.map((_, i) => ({
                    ...
                    render: ({projectSort, index}) => {
                        ...
                        return <div className={style.contributeCell}>{projectConfigs[p].name}<em>{contribution[index][p]}</em></div>
                    }
                }))
            }
        ];
        return <Table className={style.play4Owner} dataSource={dataSource} columns={columns} bordered size="small" pagination={false}/>;
    }
}
