...
function Project({
  ...
  const [_threshold, _payoff] =
    arm === Arm.Arm1 ? [commonProjectConfig.threshold, commonProjectConfig.payoff] : [threshold, payoff]
  const content = (
    <Card className={style.project}>
      ...
      <h1>
        Your contribution : <em>{yourC}</em>
      </h1>
      <h1>
        Total contribution : <em>{totalC}</em>
      </h1>
      <div className={style.btnContributeWrapper}>
        <Button type="primary" disabled={!!yourC} onClick={() => setContributeModal(true)}>
          Contribute
        </Button>
      </div>
      ...
      <Modal
        ...
        closable={false}
        footer={
          <Button type={'primary'} onClick={() => setMoreInfoModal(false)}>
            OK
          </Button>
        }
      >
      ...
      <Modal
        className={style.contributeModal}
        visible={contributeModal}
        closable={false}
        footer={
          <div>
            <Button type={'danger'} onClick={() => setContributeModal(false)}>
              CANCEL
            </Button>
            <Button
              type={'primary'}
      ...
      content={
        {
          [Treatment.Probability]: (
            <div className={style.projectPopover}>Probability to be funded : {fundProbability}</div>
          ),
    ...
    <section className={style.instruction}>
      ...
      <div className={style.userNameWrapper}>
        {playerState.username ? (
          ...
          <>
            <Input placeholder="Username" value={username} onChange={({ target: { value } }) => setUsername(value)} />
            <br />
...
function Contribute({
  ...
  return (
    <section className={style.contribute}>
      <div className={style.tokenInfo}>Your tokens : {tokenLeft}</div>
      <Row className={style.projects}>
        {projectSort.map((p, i) => (
          <Col span={8} style={{ minWidth: '18rem' }}>
            <Project
              {...projectConfigs[p]}
              {...{
                index: i,
      ...
      <Modal visible={helpModal} closable={false} onOk={() => alert('TODO')} onCancel={() => setHelpModal(false)}>
        If you have any questions or comments about the experiment, please detail them and{' '}
        <a href="mailto:hello@email.com">EMAIL US</a>.
      </Modal>
      <Modal
        visible={statisticsIndex >= 0}
        closable={false}
        footer={
          <div>
            <Button type="primary" onClick={() => setStatisticsIndex(statisticsIndex - 1)}>
              Continue
            </Button>
          </div>
        }
...
function Questionnaire({
  ...
  return (
    <section className={style.questionnaire}>
      <p>
        Thank you for completing this experiment. If you have any questions or comments about the experiment, please
        detail them below and click submit.
      </p>
...
> {
  ...
  render(): React.ReactNode {
    const {
      playerState: { projectSort, status },
      groupGameState: { contribution }
    } = this.props
