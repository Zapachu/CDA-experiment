...
> {
      ...
      while (curRound < rounds) {
        const row =
          curRound === 0
            ? [
                ...
                ps.seatNumber || '-',
                ps.mobile || '-',
                participationFee + (ps.finalProfit * s || 0),
                curRound + 1
              ]
            : ['', '', '', '', curRound + 1]
        ...
        if (curChoice) {
          row.push(
            ...
            curChoice.c1 === Choice.Wait ? choiceTerms[curChoice.c2[0]] : 0,
            curChoice.c1 === Choice.Wait ? choiceTerms[curChoice.c2[1]] : 0
          )
          const curRoundState = curGroup.rounds[curRound]
          if (curGroup.mins[curRound]) {
            row.push(
              curChoice.c ? curChoice.c : curChoice.c1,
              gameType === GameType.T2 ? (curGroup.ones[curRound] ? 1 : 0) : '-',
              curRoundState.x1,
              curRoundState.y1,
              curGroup.mins[curRound],
              ps.profits[curRound]
            )
  ...
  _rebuildGroups(
    ...
    { rounds, p, playersPerGroup, gameType, version }: ICreateParams
  ): Array<GameState.IGroup> {
  ...
  protected async playerMoveReducer(
    ...
    switch (type) {
      ...
      case MoveType.toMain: {
        ...
        const ready =
          playersInGroup.length === playersPerGroup &&
          playersInGroup.every(ps => ps.stageIndex === TestStageIndex.Wait4Others)
        if (ready) {
      ...
      case MoveType.answerMain: {
        ...
        playerState.choices[curRoundIndex] = {
          c1: params.c1,
          c2: params.c2 || []
        }
        ...
        const playersInGroup = await this.getPlayersInGroup(playerState.groupIndex)
        const ready =
          playersInGroup.length === playersPerGroup && playersInGroup.every(ps => !!ps.choices[curRoundIndex])
        if (ready) {
          ...
          switch (gameType) {
            case GameType.T1: {
              const min = playersInGroup.some(ps => ps.choices[curRoundIndex].c1 === Choice.One)
                ? Choice.One
                : Choice.Two
            ...
            case GameType.T2: {
              ...
              const min = playersInGroup.some(ps => ps.choices[curRoundIndex].c === Choice.One)
                ? Choice.One
                : Choice.Two
              groups[playerState.groupIndex].mins[curRoundIndex] = min
              playersInGroup.forEach(ps => {
