...
import { Choice, GameType, Version } from '../../config'
...
const Choice2: React.FunctionComponent<PropsType> = ({ c1, c2, onChoose, gameType, version, d = 0 }) => {
  const lang = Lang.extractLang({
    ...
    ifNot1A: [
      '如果不是“第一阶段有人选1, 且抽中表A”',
      'If no one has chosen 1 in the first action or this is not table A,'
    ],
    ok: ['确定', 'OK']
  })
  ...
  return (
    <div className={style.choice}>
      ...
      <div className={c1 === Choice.Wait ? '' : style.disabled}>
        <p>
          {version === Version.V3 ? lang.if1A : lang.if1} {lang.yourChoice}:
        </p>
        <Radio value={c2[0]} options={options} onChange={val => onChoose([val as number, c2[1]])} />
      </div>
      ...
      <div className={c1 === Choice.Wait ? '' : style.disabled}>
        <p>
          {version === Version.V3 ? lang.ifNot1A : lang.ifNot1} {lang.yourChoice}:
        </p>
        <Radio value={c2[1]} options={options} onChange={val => onChoose([c2[0], val as number])} />
      </div>
      <div className={c1 === Choice.Wait ? style.hidden : ''}>
        <p>{lang.noChoice}</p>
        <Radio
