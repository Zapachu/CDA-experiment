...
export default class MainStage extends Core.Play<
  ICreateParams,
  IGameState,
  IPlayerState,
  MoveType,
...
> {
  ...
  initState = props => {
    const {
      playerState: { groupIndex, choices },
      gameState: { groups }
    } = props
  ...
  calcDisplayData = () => {
    const {
      props: {
        playerState: { groupIndex, roundIndex },
        gameState: { groups },
        game: {
          params: { a, b, c, eL, eH, b0, b1, version }
        }
      }
    } = this
    const curGroup = groups[groupIndex]
  ...
  renderChoice2 = (c1, c2) => {
    ...
    return (
      <p>
        {lang.chooseInSecondActionLeft}
        {c2[0]}
        {lang.chooseInSecondActionRight}
        {c2[1]}
      </p>
    )
  }
  ...
  renderResult = () => {
    const {
      ...
      props: {
        frameEmitter,
        playerState: { groupIndex, roundIndex, choices, profits, finalProfit },
        gameState: { groups },
        game: {
          params: { gameType, minType }
        }
      },
      state: { c1, c2 }
    } = this
    ...
    const statisticsText =
      minType === MinType.T1 ? (
        <p>
          {lang.totalStatistics1}
          {curRoundIndex + 1}
          {lang.totalStatistics2} {curRound.x1}
          {lang.totalStatistics3}
          {curRound.y1}
          {lang.totalStatistics4}
        </p>
      ) : (
        <p>
          {lang.lowestChocieLeft}
          {curRoundIndex + 1}
          {lang.lowestChocieRight} {curGroup.mins[curRoundIndex]}
        </p>
      )
    switch (gameType) {
      case GameType.T1: {
        return (
          <>
            <p>
              {lang.yourFirstChoiceLeft}
              {curRoundIndex + 1}
              {lang.yourFirstChoiceRight} {curChoice.c1}
            </p>
            {statisticsText}
            <p>
              {lang.profitLeft}
              {curRoundIndex + 1}
              {lang.profitRight} {curProfit.toFixed(2)}
            </p>
            <p>
              {lang.totalProfitLeft}
              {curRoundIndex + 1}
              {lang.totalProfitRight} {finalProfit.toFixed(2)}
            </p>
            <Button
              ...
              onClick={() => {
                frameEmitter.emit(MoveType.advanceRoundIndex, {
                  nextRoundIndex: curRoundIndex + 1
                })
        ...
        return (
          <>
            <p>
              {lang.yourFirstChoiceLeft}
              {curRoundIndex + 1}
              {lang.yourFirstChoiceRight}{' '}
            </p>
            {this.renderChoice2(c1, c2)}
            <p style={{ margin: '30px 0' }}>
              {lang.inFirstAction} {curGroup.ones[curRoundIndex] ? lang.chose1 : lang.notChose1}
            </p>
            {curChoice.c1 !== Choice.One ? (
              <p>
                {lang.yourSecondChoice} {curChoice.c}
              </p>
            ) : null}
            {statisticsText}
            <p>
  ...
  render() {
    const {
      ...
      props: {
        frameEmitter,
        playerState: { stageIndex, roundIndex },
        game: {
          params: { gameType, version, d, rounds }
        }
      },
      state: { c1, c2 }
    } = this
