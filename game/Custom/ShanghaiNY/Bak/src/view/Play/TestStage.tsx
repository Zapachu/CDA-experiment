...
export default class TestStage extends Core.Play<
  ICreateParams,
  IGameState,
  IPlayerState,
  MoveType,
    ...
    return (
      <>
        {words.map(({ text, color, br }, i) => {
          ...
          return (
            <span key={i} className={className} style={{ marginTop: `${br}px` }}>
              {text}
            </span>
          )
        })}
  ...
  render() {
    const {
      ...
      props: {
        ...
        playerState: { stageIndex },
        game: {
          params: { gameType, version, d }
        }
      },
      state: { c1, c2, answers, tips }
    } = this
    ...
    if (stageIndex === TestStageIndex.Interface) {
      ...
      const curTest = this.Test[stageIndex]
      content = (
        <div className={style.testQuestion}>
          <h3>{lang.pageIndex(stageIndex + 1, this.Test.length)}</h3>
          <Display data={displayData} />
          <p className={style.desc}>{this.joinWords(curTest.desc)}</p>
          <ul>
            {curTest.questions.map(({ title, options }, i) => (
              <li key={i} style={{ paddingBottom: '20px' }}>
                <p className={tips[i] === Tip.Wrong ? style.tipWrong : ''}>
                  {this.joinWords(title)} {tips[i] === Tip.Wrong ? <span>{lang.wrong}</span> : null}
                </p>
                <Radio
                  options={typeof options === 'function' ? options(displayData, d) : options}
                  value={answers[i] || ''}
          ...
          <Button width={ButtonProps.Width.small} label={lang.confirm} onClick={this.submit} />
        </div>
      )
    }
    return <section className={style.testStage}>{content}</section>
  }
}
