...
const Choice1: React.FunctionComponent<PropsType> = ({ c1, onChoose, gameType, version, d = 0 }) => {
  ...
  const options =
    gameType === GameType.T1
      ...
      ? [
          ...
          { label: lang.chooseWait, value: Choice.Wait }
        ]
      : [
          {
            label: lang.choose1,
          ...
          {
            label:
              gameType === GameType.T2 && d > 0
                ? `${lang.chooseWait}${lang.feeLeft}${d}${lang.feeRight}`
                : lang.chooseWait,
            value: Choice.Wait
          }
        ]
  return (
    <div className={style.choice}>
      ...
      <Radio value={c1} options={options} onChange={val => onChoose(val as number)} />
    </div>
  )
}
