...
> {
  ...
  renderEndStage = () => {
    const {
      ...
      props: {
        game: {
          params: { s, participationFee }
        },
        playerState: { finalProfit }
      }
    } = this
    return (
      <section className={style.endStage}>
        <p>
          {lang.totalPoint}
          {isNaN(finalProfit) ? '-' : finalProfit.toFixed(2)}
        </p>
        <p>
          {lang.totalProfit}
          {isNaN(finalProfit * s + participationFee) ? '-' : (finalProfit * s + participationFee).toFixed(2)}
        </p>
      </section>
    )
  }
  ...
  renderSeatNumberStage = () => {
    ...
    return playerState.seatNumber ? (
      <MaskLoading label={lang.wait4StartMainTest} />
    ) : (
