...
export default function Choice2({
  ...
  const questions: Array<{
    question: string
    options: Array<{ label: string; value: Choice }>
    ...
    .map((_, i) => ({
      ...
      options: {
        [Mode.HR]:
          (c1 === Choice.Wait && i === playersPerGroup) || (c1 === Choice.One && i === 0) ? null : ONE_TWO_OPTIONS,
        [Mode.LR]:
          (c1 === Choice.Wait && i === playersPerGroup) || (c1 === Choice.Two && i === 0) ? null : ONE_TWO_OPTIONS,
        [Mode.BR]:
          c1 === Choice.One ? (i === 0 ? null : ONE_TWO_OPTIONS) : i === playersPerGroup ? null : ONE_TWO_OPTIONS
      }[mode]
    }))
  const questionRows = []
  questions.forEach(({ question, options }, i) => {
    ...
    questionRows.push(
      <tr>
        <td>
          <p>
            ({questionRows.length + 1}) {question}
          </p>
        </td>
        <td>
          <Radio.Group
          ...
          >
            {(mode === Mode.BR || c1 === Choice.Wait ? options : OK_OPTIONS).map(({ label, value }) => (
