...
> {
  ...
  renderChoice2 = (c1, c2) => {
    ...
    return (
      <section className={style.choice2Result}>
        <p>
          {lang.chooseInAction1}
          <b>{lang.firstAction}</b>
          {lang.chooseInAction2}
          {choice2Lang[c1]}
        </p>
        {c2.some(c => [Choice.One, Choice.Two].includes(c)) ? (
          <>
            ...
            <table className={style.yourChoice2}>
              ...
              {Array(playersPerGroup + 1)
                .fill(null)
                .map((_, i) => {
                  const c = c2[i]
                  return [Choice.One, Choice.Two].includes(c) ? (
                    <tr key={i}>
                      <td>
                        {i}
                        {lang.players}
                        {chooseLabel[0]}&nbsp;,&nbsp;{playersPerGroup - i}
                        {lang.players}
                        {chooseLabel[1]}
                      </td>
                      <td>{choice2Lang[c]}</td>
  ...
  renderResult = () => {
    ...
    return (
      <>
        ...
        {curChoice.c2.some(c => [Choice.One, Choice.Two].includes(c)) ? (
          <p>
            {lang.yourSecondChoice1}
            <b>{lang.secondAction}</b>
            {lang.yourSecondChoice2} {curChoice.c}
          </p>
        ) : null}
        <br />
        {min === Min.A ? (
          <p>
            {lang.roundResult0}
            {curRoundIndex + 1}
            {lang.roundResult1} {curRound.min}
          </p>
        ) : (
          <p>
            {lang.roundResult2}
  ...
  render() {
    const {
      ...
      props: {
        ...
        game: {
          params: { playersPerGroup, mode, d, rounds }
        }
      },
      state: { c1, c2 }
    } = this
