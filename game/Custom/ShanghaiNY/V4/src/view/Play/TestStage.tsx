        ...
        <div className={style.testQuestion}>
          ...
          <ul>
            {curTest.questions.map(({ title, options }, i) => (
              <li key={i} style={{ paddingBottom: '1.5rem' }}>
          ...
          {this.someWrongTips ? (
            <Button style={btnStyle} onClick={() => this.choseAgain()}>
              {lang.choseAgain}
            </Button>
          ) : (
            <Button type="primary" style={btnStyle} onClick={() => this.confirmTest()}>
              {lang.confirm}
            </Button>
          )}
        </div>
