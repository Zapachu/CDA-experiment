...
> {
  ...
  render(): React.ReactNode {
    const {
      lang,
      props: { game, travelStates },
      state: { activeMoveSeq, activeTabIndex }
    } = this
      ...
      <section className={style.result4Owner}>
        ...
        >
          <div className={style.resultWrapper}>
            <table className={style.resultTable}>
              <tbody>
                ...
                {Object.values(travelStates[travelStates.length - 1].playerStates)
                  .sort((a, b) => Number(a.seatNumber) - Number(b.seatNumber))
                  .map(({ seatNumber, finalProfit }, i) => (
                    <tr key={i}>
                      <td>{seatNumber === undefined ? lang.unknown : seatNumber}</td>
                      <td>
                        {isNaN(finalProfit * s + participationFee)
                          ? '-'
                          : (finalProfit * s + participationFee).toFixed(2)}
                      </td>
                    </tr>
                  ))}
              </tbody>
          ...
          <div className={style.timeTravelWrapper}>
            ...
            <div>{travelState.gameState ? <Play4Owner {...{ game, ...travelState, frameEmitter: null }} /> : null}</div>
          </div>
          {/* <div>
                    <div>
                        <label>发送奖励给</label>
