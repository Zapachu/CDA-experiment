...
> {
  ...
  _rebuildGroups(
    ...
    { rounds, p, playersPerGroup }: ICreateParams
  ): Array<IGameGroupState> {
  ...
  protected async playerMoveReducer(
    ...
    switch (type) {
      ...
      case MoveType.answerMain: {
        ...
        playerState.choices[roundIndex] = {
          c1: params.c1,
          c2: params.c2 || []
        }
        ...
        if (ready) {
          ...
          playersInGroup.forEach(ps => {
            ...
            curChoice.c = curChoice.c2.some(c => [Choice.One, Choice.Two].includes(c))
              ? curChoice.c2[mode === Mode.LR ? two1 : one1]
              : curChoice.c1
          })
