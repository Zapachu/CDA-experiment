...
export class Matrix extends React.Component<IMatrixProps, IMatrixState> {
  ...
  render() {
    const { roleNames, options, matrix, dices, activeRow, activeCol } = this.props,
      { dieRoll } = this.state
    return (
      <section className={`${style.matrix} ${!this.props.dieRoll || dices.includes(dieRoll) ? '' : style.disabled}`}>
        {!dices ? null : (
          <div>
            ...
            <tr>
              <td className={style.svgWrapper}>
                <svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0,0,100,32">
                  <text x="8" y="32" className={style.roleName}>
                    {roleNames[Role.A]}
                  </text>
                  <text x="55" y="12" className={style.roleName}>
                    {roleNames[Role.B]}
                  </text>
