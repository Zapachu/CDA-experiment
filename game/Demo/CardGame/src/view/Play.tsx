...
> {
  ...
  render(): React.ReactNode {
    const {
      ...
      props: {
        ...
        playerState: { playerStatus, pairId }
      }
    } = this
    if (!pairId) {
  ...
  renderCardGame() {
    const {
      props: {
        gameState: { periods, periodIndex },
        playerState: { role, playerStatus, periodIndices, profit },
        frameEmitter
      },
      state: { choice }
    } = this
    ...
    return (
      <section className={style.cardGame}>
        <Header
          {...{
            period: periodIndex,
              ...
              <React.Fragment>
                ...
                <section key="roundDetailWrapper" className={style.roundDetailWrapper}>
                  <ul className={`${style.roleA} ${role === Role.A ? style.active : ''}`}>
                    <li>
                      Player1's Move : <em>{cardGame.cards[theLastPeriod.aChoice]}</em>
                    </li>
                    <li>
                      Player1's Earnings : <em>{theLastPeriod.aEarning}</em>
                    </li>
                  </ul>
                  <ul className={`${style.roleB} ${role === Role.B ? style.active : ''}`}>
                    <li>
                      Player2's Move : <em>{cardGame.cards[theLastPeriod.bChoice]}</em>
                    </li>
                    <li>
                      Player2's Earnings : <em>{theLastPeriod.bEarning}</em>
                    </li>
                  </ul>
                </section>
              </React.Fragment>
  ...
  renderLeftRightGame() {
    const {
      props: {
        frameEmitter,
        gameState: { periods, periodIndex },
        playerState: { playerStatus, role, periodIndices, profit }
      },
      state: { choice }
    } = this
    const theLastPeriod = periods.find(({ index }) => index === periodIndices[periodIndices.length - 1]) || {
    ...
    return (
      <section className={style.LRGame}>
        <Header
          {...{
            period: periodIndex,
          ...
          {playerStatus === PlayerStatus.result ? (
            [
              ...
              <section key="roundDetailWrapper" className={style.roundDetailWrapper}>
                <ul className={`${style.roleA} ${role === Role.A ? style.active : ''}`}>
                  <li>
                    Seeker's Move : <em>{LRGame.options[theLastPeriod.aChoice]}</em>
                  </li>
                  <li>
                    Seeker's Earnings : <em>{theLastPeriod.aEarning}</em>
                  </li>
                </ul>
                <ul className={`${style.roleB} ${role === Role.B ? style.active : ''}`}>
                  <li>
                    Hider's Move : <em>{LRGame.options[theLastPeriod.bChoice]}</em>
                  </li>
                  <li>
                    Hider's Earnings : <em>{theLastPeriod.bEarning}</em>
                  </li>
                </ul>
              </section>
            ]
