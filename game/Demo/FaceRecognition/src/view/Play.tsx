...
> {
  ...
  startRecognize() {
    this.props.frameEmitter.emit(MoveType.getQiniuConfig, {}, qiNiuUploadToken =>
      this.setState(
        { qiNiuUploadToken, recognizing: true },
  ...
  recognize() {
    ...
    canvas.toBlob(blob => {
      const imageName = `images/azure/faces/${Math.random()
        .toString(32)
        .substr(2)}.png`
      ...
      observable.subscribe(
        ...
        () => null,
        () =>
          frameEmitter.emit(MoveType.recognize, { imageName }, (resultArray: TResultItem[]) => {
  ...
  render(): React.ReactNode {
    const {
      lang,
      videoRef,
      state: { resultArray, videoMetaData, recognizing }
    } = this
    return (
      <section className={style.play}>
        <div className={style.mainContent}>
              ...
              <React.Fragment>
                ...
                >
                  {resultArray.map(
                    (
                      {
                        faceRectangle: { left, top, height, width },
                        faceLandmarks: {
                          eyeLeftBottom,
                          eyeLeftTop,
                          eyeLeftInner,
                      ...
                      <React.Fragment key={i}>
                        <polygon
                          points={`${left},${top} ${left},${top + height} ${left + width},${top + height} ${left +
                            width},${top}`}
                        />
                        <polygon
          ...
          <div className={style.btnWrapper}>
            {recognizing ? (
              <>
                <Button type="danger" onClick={() => this.stopRecognize()}>
                  {lang.pause}
                </Button>
                <Button onClick={() => this.recognize()}>{lang.recognize}</Button>
              </>
            ) : (
              <Button type="primary" onClick={() => this.startRecognize()}>
                {lang.start}
              </Button>
            )}
          </div>
        ...
        <List
          ...
          renderItem={({ faceAttributes: { age, gender, emotion } }: TResultItem) => (
            <List.Item>
              <List.Item.Meta
                ...
                description={
                  <span>
                    {Object.entries(emotion)
                      ...
                      .map(([tag, v]) =>
                        v > 0.01 ? (
                          <Tag color={v > 0.5 ? 'green' : v > 0.3 ? 'blue' : ''} key={tag}>
                            {tag}[{v}]
                          </Tag>
                        ) : null
                      )}
