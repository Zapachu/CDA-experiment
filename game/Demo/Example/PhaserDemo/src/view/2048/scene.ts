...
export class MainGame extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet(assetName.tiles, asset.tiles, {
  ...
  moveTile(fromCoord: TileCoord, toCoord: TileCoord) {
    ...
    this.tweens.add({
      ...
      y: tileY(toCoord.r),
      duration:
        (CONST.tweenDuration * (Math.abs(fromCoord.r - toCoord.r) + Math.abs(fromCoord.c - toCoord.c))) /
        (CONST.row + CONST.col),
      onComplete: () => {
      ...
      ...(upgraded
        ? {
            alpha: 0.5,
            scaleX: { value: 0.5, duration: 50, delay: 50, yoyo: true },
            scaleY: { value: 0.5, duration: 50, delay: 50, yoyo: true }
          }
        : null)
    })
  }
  ...
  layout() {
    const fontSize = 22
    this.add.sprite(span(0.8), span(1), assetName.score)
    this.scoreText = this.add.text(span(0.8), span(1), this.score.toString(), {
      fontSize
    })
    ...
    this.add
      .sprite(span(3), span(1), assetName.restart)
      .setInteractive()
      .on('pointerdown', () => this.scene.start(SceneName.mainGame))
    for (let r = 0; r < CONST.row; r++) {
      this.tileMatrix[r] = []
      for (let c = 0; c < CONST.col; c++) {
  ...
  addTile() {
    ...
    this.tileMatrix.forEach((row, r) => row.forEach(({ value }, c) => (value === 0 ? emptyCoord.push({ r, c }) : null)))
