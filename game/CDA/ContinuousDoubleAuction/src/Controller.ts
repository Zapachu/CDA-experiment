...
> {
  ...
  async init() {
    ...
    robot.forEach((_, i) => setTimeout(async () => await this.startRobot(`Robot_${i}`), Math.random() * 3000))
    return this
  }
  ...
  async initPlayerState(actor: IActor): Promise<TPlayerState<IPlayerState>> {
    ...
    playerState.point = 0
    playerState.phases = this.game.params.phases.map(({ templateName }) =>
      templateName === phaseNames.mainGame ? { periodProfit: 0 } : {}
    )
  ...
  async calcPeriodProfit() {
    ...
    for (const userId in playerStates) {
      ...
      trades.forEach(({ reqId, resId }) => {
        ...
        const unitProfit =
          positions[positionIndex].role === ROLE.Buyer ? privateCost - reqOrder.price : reqOrder.price - privateCost
      ...
      playerStates[userId].point = Number(
        (playerStates[userId].point + periodProfit / Number(positions[positionIndex].exchangeRate || 1)).toFixed(2)
      )
    }
  }
  ...
  async onGameOver() {
    ...
    const events = await this.getMoveEvent({}, { sort: { createAt: 1 } })
    events
      .map(e => e.data)
      .forEach((data, i) => {
        const {
    ...
    const resultData: Array<Array<any>> = [
      [
        ...
        'TraderType: 0-human,1-zip,2-gd',
        'ValueCost',
        'BidAsk',
        'Trade',
        'TradeOrder',
        'TradeTime',
        'TradeType: 1-buyer_first,2-seller_first',
        'Price',
        'Profit',
        'PartnerSubject',
        'PartnerBox',
    ...
    const logData: Array<Array<any>> = [
      [
        ...
        'TraderType: 0-human,1-zip,2-gd',
        'ValueCost',
        'EventType: 1-rejected,2-entered,3-traded,4-cancelled',
        'EventNum',
        'EventTime',
        'EventEndTime',
        'MaxBid',
    ...
    const profitData: Array<Array<any>> = [
      ['Period', 'Subject', 'PeriodProfit', 'MarketProfit', 'ShowUpFee', 'ExchangeRate', 'Money']
    ]
    Object.values(players).forEach(period => {
      Object.values(period).forEach(box => {
        Object.values(box).forEach(params => {
  ...
  //region util
  insertMoveEvent(data: Record<string, any>): void {
    new Model.FreeStyleModel({
      data,
      game: this.game.id,
      key: DBKey.moveEvent
    }).save(err => {
  ...
  async getMoveEvent(query = {}, options = { sort: null }): Promise<Array<{ data: EventParams }>> {
    ...
    if (options.sort) {
      return Model.FreeStyleModel.find(queryObj)
        .lean()
        .sort(options.sort)
        .exec()
    }
      ...
      case MoveType.assignPosition: {
        Object.values(playerStates).forEach(async playerState => {
          const positionIndex = (playerState.actor.type === Actor.serverRobot
            ? this.positionStack.robot
            : this.positionStack.player
          ).pop()
          if (positionIndex === undefined) {
            return Log.d('角色已分配完')
          }
  ...
  protected async playerMoveReducer(
    ...
    switch (type) {
      ...
      case MoveType.submitOrder: {
        ...
        const orderDict = this.getOrderDict(gameState),
          { buyOrderIds, sellOrderIds } = gameState.phases[gamePhaseIndex],
      ...
      case MoveType.rejectOrder: {
        const { price } = params
        const { positions } = this.game.params.phases.filter(
            ph => ph.templateName === phaseNames.assignPosition
          )[0].params,
