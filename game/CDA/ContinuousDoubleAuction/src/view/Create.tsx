    ...
    const phases = [
      ...
      },
      {
        ...
        params: {
          ...
          time2ReadInfo: 15,
          unitLists: marketPositions.map(({ role }) =>
            role === ROLE.Buyer ? '325 325 305 260 220' : '185 185 235 260 270'
          ),
      ...
      <section className={style.create}>
        <ul className={style.phases}>
          {phases.map(({ templateName, params }, i) => {
            ...
            return (
              <li className={style.phase} key={`${i}-${templateName}`}>
                <label className={style.phaseTitle}>
                  {i + 1} - {Lang.extractLang({ name: localePhaseNames[name] }).name}
                </label>
                <div className={style.createWrapper}>
                  ...
                  />
                  {name === phaseNames.mainGame ? (
                    <ul className={style.marketBtns}>
                      {i > 1 ? (
                        <li className={style.btnRemove} onClick={() => this.removePhase(i)}>
                          {lang.remove}
                        </li>
                      ) : null}
