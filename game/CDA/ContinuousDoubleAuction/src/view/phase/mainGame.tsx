...
export function UnitList({ unitList, editable, onChange = (...args) => {} }) {
  return (
    <section className={style.unitList}>
      <input
        {...{
...
class Create extends BasePhase.Create<ICreateState> {
  ...
  render() {
    const {
      lang,
      props: { params, updateParams },
      state: { minStartTime, maxStartTime }
    } = this
    return (
      <section className={`${style.mainGame} ${style.createContent}`}>
        <ul className={style.baseFields}>
          <li>
            <Label label={lang.practicePhase} />
            <Switch
              checked={params.practicePhase}
                ...
                <td>
                  <div className={style.roleSwitcher}>
                    {getEnumKeys(ROLE).map(key => (
                      <a key={key} className={ROLE[key] === role ? style.active : ''}>
                        {lang[key]}
                      </a>
                    ))}
                  </div>
...
class Play extends BasePhase.Play<IPlayState> {
  lang = Lang.extractLang({
    ...
    invalidBuyPrice: [
      '订单价格需在市场最高买价与当前物品价值之间',
      'Order price must be between your private value and the highest buy price in the market'
    ],
    invalidSellPrice: [
      '订单价格需在当前物品成本与市场最低卖价之间',
      'Order price must be between the lowest buy price in the market and your private cost'
    ],
  ...
  render() {
    ...
    return (
      <section className={`${style.mainGame} ${style.playContent}`}>
        <ul className={style.header}>
          <li>
            {lang.period} :{' '}
            <em>
              {gamePhaseIndex}/{game.params.phases.length - 2}
            </em>
          </li>
          <li>
            {lang.role} : <em>{lang[ROLE[role]]}</em>
          </li>
          <li>
            {lang.timeLeft} : <em>{timer < time2ReadInfo ? '   ' : timeLeft > 0 ? timeLeft : 0}</em>s
          </li>
        </ul>
          ...
          <section className={style.body}>
            <div className={style.personalPanel}>
                ...
                <div className={style.infoText}>
                  <ul>
                    <li>
                      {lang.totalProfit}
                      <em>
                        {playerStatePhases.map(({ periodProfit = 0 }) => periodProfit).reduce((m, n) => m + n, 0)}
                      </em>
                    </li>
                    <li>
                      {lang.currentProfit}
                    ...
                    <li>
                      {lang.participationFee}
                      <em>{participationFee}</em>
                    </li>
                    <li>
                      {lang.exchangeRate}
                      <em>{exchangeRate}</em>
              ...
              <section className={style.yourTrades}>
                ...
                <div className={style.tradeListWrapper}>{tradeListFragment}</div>
              </section>
            </div>
  ...
  renderOrderList(orderDict: { [id: number]: GameState.IOrder }, marketOrderIds: Array<number>, shoutRole) {
    ...
    return (
      <table className={style.orderList}>
        <thead>
          <tr>
        ...
        <tbody>
          {marketOrderIds.map((orderId, i) => {
            ...
            return (
              <tr key={i} className={isMine ? style.active : ''}>
                <td>{orderX.price}</td>
                <td className={style.btnCancelWrapper}>
                  {i && isMine ? (
                    <a className={style.btnCancel} onClick={() => frameEmitter.emit(MoveType.cancelOrder)}>
                      Cancel
                    </a>
                  ) : null}
                </td>
  ...
  renderTradeList(
    orderDict: { [id: number]: GameState.IOrder },
    trades: Array<GameState.GamePhaseState.ITrade>
    ...
    const fragment = (
      <table className={style.tradeList}>
        ...
        <tbody>
          {myTrades.map(({ price, privateCost }, i) => (
            <tr key={i}>
              <td>{i + 1}</td>
