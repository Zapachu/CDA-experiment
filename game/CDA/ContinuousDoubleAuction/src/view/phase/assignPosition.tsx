...
class Create extends BasePhase.Create {
  ...
  render() {
    ...
    return (
      <section className={`${style.assignPosition} ${style.createContent}`}>
              ...
              <tr key={positionIndex}>
                ...
                <td>
                  <div className={style.centerWrapper}>
                    <BtnGroup
                      value={roleKeys.findIndex(key => role === ROLE[key])}
                      options={roleKeys.map(key => lang[key])}
                      onChange={i =>
                        this.updatePosition(positionIndex, position => ({
                          ...position,
                ...
                <td>
                  <div className={style.centerWrapper}>
                    <BtnGroup
                      value={identityKeys.findIndex(key => identity === IDENTITY[key])}
                          ...
                          <div>
                            ...
                            <input
                              {...{
                                ...
                                onChange: ({ target: { value: exchangeRate } }) =>
                                  this.updatePosition(positionIndex, position => ({
                                    ...position,
                                    exchangeRate
                                  }))
                            ...
                            <input
                              {...{
                                ...
                                onChange: ({ target: { value: k } }) =>
                                  this.updatePosition(positionIndex, position => ({ ...position, k }))
                              }}
                            />
                          </div>
                        )
                            ...
                            <input
                              {...{
                                ...
                                onChange: ({ target: { value: interval } }) =>
                                  this.updatePosition(positionIndex, position => ({ ...position, interval }))
                              }}
                            />
                            <label>s</label>&nbsp;
                            <label>{lang.reactionType} : </label>
                            <BtnGroup
                              value={reactionTypeKeys.findIndex(key => extraConfig.reactionType === ReactionType[key])}
                ...
                <td onClick={() => this.removePosition(positionIndex)}>
                  <span>❌</span>
                </td>
              </tr>
            ))}
  ...
  addPosition() {
    ...
    const positions = [...params.positions, { ...params.positions.slice().pop() }]
...
class Info extends Create {
  render() {
    ...
    return (
      <section className={`${style.assignPosition} ${style.infoContent}`}>
        <ul className={style.baseFields}>
          <li>
            <Label label={lang.participationFee} />
                ...
                <td>
                  <ul className={style.identitySelector}>
                    {getEnumKeys(IDENTITY).map(key => (
                      <li key={key} className={IDENTITY[key] === identity ? style.active : ''}>
                        {key}
                      </li>
                    ))}
                  </ul>
...
class Play extends BasePhase.Play<IPlayState> {
  ...
  render() {
    const {
      ...
      props: {
        game,
        playerState: { positionIndex, status }
      }
    } = this
    return positionIndex === undefined ? (
      ...
      <section className={`${style.assignPosition} ${style.playContent}`}>
        <p>
          {lang.toEnterMarket}
          <em>{lang[ROLE[game.params.phases[0].params.positions[positionIndex].role]]}</em>
        </p>
