...
class ApplyAnimation extends React.Component<PropType, StateType> {
  ...
  applyForSchool = (schoolIndex: number) => {
    ...
    this.setState(partialState, () => {
      setTimeout(() => {
        if (next) {
          return isMe ? this.finish(currentPlayer.admission) : this.nextPlayer(currentPlayer.admission)
        }
  ...
  nextPlayer = (school: SCHOOL) => {
    ...
    this.toast(school === SCHOOL.none ? '未被录取' : `被${SCHOOL_NAME[school]}录取`)
    setTimeout(() => {
      this.setState(
        {
          ...
          {Object.values(players).map((player, i) => {
            ...
            return (
              <li
                ...
                style={players[i].token === myToken ? { backgroundColor: '#F25D7D' } : {}}
              >
                <p>组员:{i - 2}</p>
                <p>成绩:{player.score}</p>
      ...
      <div className={style.schools}>
        <ul className={style.applies}>
          {currentPlayer.schools.map((school, i) => {
            return (
              <li key={school} className={schoolStatus[school] === SCHOOL_STATUS.default ? style.disabled : ''}>
          ...
          return (
            <ul key={pies.toString()} className={style.schoolPie}>
              {pies.map(school => {
                ...
                return (
                  <li key={school}>
                    <img src={currentPlayer.schools.includes(school) ? SCHOOL_PIE_ACTIVE : SCHOOL_PIE} />
  ...
  renderToast = () => {
    ...
    return (
      <div className={style.toast} style={{ visibility: msg ? 'visible' : 'hidden' }}>
        <p>{msg}</p>
      </div>
    )
  }
