...
> {
  ...
  apply = (schools: Array<SCHOOL>) => {
    this.props.frameEmitter.emit(MoveType.shout, { schools }, err => Toast.warn(err))
  }
  ...
  finishApplying = (school: SCHOOL) => {
    ...
    this.setState({ showModal: MODAL.admission, admission: school }, () => {
      setTimeout(() => frameEmitter.emit(MoveType.result, {}, url => (window.location.href = url)), 2000)
    })
  }
  ...
  _renderReady2Apply = (schools: Array<SCHOOL>) => {
    ...
    return (
      <>
        ...
        <Button className={style.button} label="提交" onClick={() => this.apply(schools)} />
      </>
    )
  }
    ...
    return (
      <>
        ...
        <div className={style.buttonContainer}>
          ...
          <Button label="开始填报" onClick={() => this.setState({ schools: [] })} />
        </div>
      </>
    )
  }
  ...
  renderContent = () => {
    ...
    switch (status) {
      ...
      case STATUS.beforeChoose: {
        ...
        content = this._renderReady2Choose(playerState.score, playerState.scores)
        break
      }
    ...
    return (
      <>
        <Teacher msg={msg} onGameRuleClick={() => this.setState({ showModal: MODAL.rule })} />
        {content}
      </>
    )
  }
  ...
  _renderRuleModal = () => {
    return (
      <div className={style.ruleModal}>
        ...
        <img src={CLOSE} onClick={() => this.setState({ showModal: undefined })} />
      </div>
    )
  }
...
const Ready2Choose: React.SFC<{ score: number; scores: Array<number> }> = ({ score, scores }) => {
