...
const resultHtmlStr = fs.readFileSync(resolve(__dirname, '../asset/result.html')).toString()

const router = Router()
  .use('/result/static', Express.static(resolve(__dirname, '../asset'), { maxAge: '10d' }))
  ...
  .get('/onceMore/:gameId', async (req: any, res: Response) => {
    const result = await RedisCall.call<Trial.Done.IReq, Trial.Done.IRes>(Trial.Done.name, {
      userId: req.user._id,
      namespace
    })
    if (result) {
