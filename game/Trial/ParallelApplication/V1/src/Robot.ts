...
import { MoveType, PushType, ICreateParams, IGameState, IMoveParams, IPlayerState, IPushParams, SCHOOL } from './config'
...
> {
  async init() {
    ...
    this.frameEmitter.on(PushType.robotShout, () => {
      ...
      setTimeout(() => this.frameEmitter.emit(MoveType.shout, { schools }), 1000)
    })
    return this
  }
}
