...
> {
  ...
  protected async playerMoveReducer(
    ...
    switch (type) {
      ...
      case MoveType.shout: {
        ...
        if (playerStateArray.length === groupSize && playerStateArray.every(ps => ps.schools !== undefined)) {
  ...
  private invalidParams(params: IMoveParams): boolean {
    ...
    return (
      ...
      schools.some(s => {
        return typeof s !== 'number' || s < SCHOOL.beijingUni || s > SCHOOL.zhongshanUni
      })
    )
  }

  private processGameState(gameState: IGameState, playerStates: Array<TPlayerState<IPlayerState>>) {
    const enrollment = {
      [SCHOOL.beijingUni]: 0,
      [SCHOOL.qinghuaUni]: 0,
      [SCHOOL.renminUni]: 0,
  ...
  private async checkOldVersion(gameState: TGameState<IGameState>, userId: string, cb: IMoveCallback) {
