...
export class MainGame extends Phaser.Scene {
  ...
  create() {
    ...
    CONST.emitter.emit(MoveType.prepare, {}, ({ env, life, garbageIndex, index, status, score }) => {
      ...
      if (index === undefined) {
        this.add
          .text(this.sys.canvas.width >> 1, this.sys.canvas.height - 240, '加入游戏失败！', {
            fontSize: '28px',
            color: '#da2422'
          })
          .setOrigin(0.5)
...
class BtnSkip {
  ...
  private constructor(private scene: Phaser.Scene, private onClick: (pointer?: IPointer) => void) {
    ...
    this.container = scene.add
      .container(stageWidth >> 1, stageHeight - 225, [btnSkip, btnSkipLabel, particles, mask])
      .setDepth(10)
  }
  ...
  static update() {
    const [btnSkip, , particles, mask] = this.instance.container.getAll() as [
...
class Player {
  ...
  private constructor(
    ...
    private onDrag: (pointer: IPointer) => void,
    private onDragEnd: (pointer: IPointer) => void,
    life: number,
    garbageIndex: number
  ) {
    scene.anims.create({
    ...
    const container = scene.add.container(Player.InitPosition.x, Player.InitPosition.y),
      shadowSprite = scene.add
        .graphics({
          x: 0,
          y: 150,
          fillStyle: { color: 0x000, alpha: 0.5 }
        })
        .fillCircle(0, 0, 100)
        .setScale(1, 0.25),
...
class Can {
  ...
  private constructor(public type: GarbageType, private scene: Phaser.Scene, onClick: (pointer: IPointer) => void) {
    ...
    this.container = scene.add.container(Can.cfg.baseX + this.type * Can.cfg.xStep, Can.cfg.baseY, [
      bodySprite,
      coverSprite,
      nameText,
      feedbackSprite
...
class Env {
  ...
  private constructor(private scene: Phaser.Scene, private n: number = 0) {
    ...
    const stripGraphics = scene.add.graphics({ lineStyle: { width: w } }),
      scoreText = scene.add
        .text(x, y - 90, n.toString(), {
          fontFamily: 'raster',
          fontSize: '60px'
        })
        .setStroke('#fff', 6)
        .setOrigin(0.5),
      scoreLabel = scene.add
        .text(x, y - 10, '环境评分', {
          fontSize: '36px',
          fontFamily: 'Open Sans'
        })
    ...
    this.container = scene.add.container(scene.sys.canvas.width >> 1, scene.sys.canvas.height - 145, [
      dumpSprite,
      bg,
      stripGraphics,
      scoreText,
  ...
  update(n: number) {
    ...
    const { container } = this,
      [dumpSprite, , stripGraphics, scoreText, scoreLabel] = container.getAll() as [
        Phaser.GameObjects.Sprite,
        Phaser.GameObjects.Graphics,
        Phaser.GameObjects.Graphics,
    ...
    const color = Phaser.Display.Color.HSLToColor((0.12 * n) / 360, 0.8, 0.43).color,
    ...
    if (n) {
      ...
      stripGraphics.slice(
        x + window.Math.cos(angle) * r,
        y + window.Math.sin(angle) * r,
        w >> 1,
        angle,
        Math.PI + angle
      )
  ...
  feedback(n: number) {
    ...
    feedBackText.setPosition(x + 2 * (Math.random() - 0.5) * r, y - r / 2)
...
class Garbage {
  ...
  private constructor(private scene: Phaser.Scene, private n: number = 0) {
    ...
    this.garbageText = scene.add
      .text(textX, textY, n.toString(), {
        fontFamily: 'raster',
        fontSize: '48px',
        color: '#999'
      })
...
class LifeStrip {
  ...
  private constructor(private scene: Phaser.Scene, private n: number = 100) {
    const alpha = 0.8
    const bg = scene.add.graphics({ fillStyle: { color: 0x020a1e, alpha } }).fillRoundedRect(-165, -210, 320, 28, 14),
  ...
  static show(scene: Phaser.Scene, str: string) {
    ...
    const textBg = scene.add
      .graphics({
        x: stageWidth,
        y,
        fillStyle: { color: 0x000, alpha: 0.6 }
      })
      .fillRoundedRect(-32, -10, text.width + 64, 56, 26)
      .setDepth(-1)
    scene.tweens.add({
      targets: [textBg, text],
      x: -stageWidth,
...
class Score {
  ...
  feedback(n: number) {
    const feedBackText = this.feedbackTween.targets[0] as Phaser.GameObjects.Text
    feedBackText.setPosition(this.scene.sys.canvas.width - 245 + 2 * (Math.random() - 0.5) * 80, 225)
    feedBackText.setText(`+${n}`)
    this.feedbackTween.play()
  }
}
...
class Guide {
  ...
  private constructor(private scene: Phaser.Scene, private onDone: () => void) {
    ...
    this.container = scene.add
      .container(scene.sys.canvas.width >> 1, 595, [
        ...
        scene.add
          .text(0, -40, '超时未分类或点击"随地乱扔"视为污染环境行为\n会造成对环境总分的下降,但是不会消耗体力值', {
            fontFamily: 'Open Sans',
            fontSize: '28px',
            color: '#666'
          })
      ...
      .setDepth(20)
    btnSkip.setInteractive().on('pointerdown', () => btnSkip.setScale(0.9))
    btnSkip.setInteractive().on('pointerup', () => this.done())
    btnNext.setInteractive().on('pointerdown', () => btnNext.setScale(0.9))
