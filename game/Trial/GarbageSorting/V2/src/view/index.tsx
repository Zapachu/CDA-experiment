  ...
  return (
    <div
      ...
      style={{
        ...
        position: 'fixed',
        top: '0',
        background: `url(${require('./asset/bg.png')})`
      }}
    />
  )
}
...
function ResultStage({ gameState: { env, sorts }, playerState }: TProps) {
  ...
  let myRank = 0
  const rowData: IRowData[] = sorts
    .map((sort, playerIndex) => {
      const rightAmount = sort.filter((t, i) => t === GarbageConfig[i].type).length,
        ...
        score =
          ~~(life / 5) +
          rightAmount * CONST.rightScore +
          (GarbageConfig.length - rightAmount - skipAmount) * CONST.wrongScore +
          envProfit
      return {
        playerIndex,
        rightAmount,
  ...
  const [r, g, b] = hslToRgb((0.12 * env) / 360, 0.8, 0.43),
    color = `rgb(${r},${g},${b})`
  let envLevelLabel = '良好'
  ...
  const d = ((CONST.maxEnv - env) / CONST.maxEnv) * Math.PI,
    w = 16,
  ...
  return (
    <section className={style.result}>
      <svg viewBox="0,0,400,200">
        <path
          d="M100 150 A 100 100 0 0 1 300 150"
          fill="transparent"
          strokeWidth={w}
          strokeLinecap="round"
          stroke="#d3d3d3"
        />
        <path
          d={`M100 150 A 100 100 0 0 1 ${200 + 100 * Math.cos(d)} ${150 - 100 * Math.sin(d)}`}
          fill="transparent"
          strokeWidth={w}
          strokeLinecap="round"
          stroke={color}
        />
        <text x="200" y="130" font-family="raster" font-size="40" fill={color} text-anchor="middle">
          {env}
        </text>
        {time2End ? (
          <text x="200" y="190" fill="#666" text-anchor="middle">
            等待其它玩家，实验将在&nbsp;{time2End}s&nbsp;内结束
          </text>
        ) : null}
      </svg>
      ...
      <div className={style.playerTableWrapper}>
        <table className={style.playerTable}>
          ...
          <tbody>
            {rowData.map(({ rank, playerIndex, rightAmount, skipAmount, life, score }) => (
              <tr className={rank === myRank ? style.myRank : ''}>
                <td>
                  <span className={rank < 3 ? style.medal : null} style={{ backgroundPositionX: rank * -16.5 }}>
                    {rank + 1}
                  </span>
                </td>
                ...
                <td>
                  {rightAmount}/{GarbageConfig.length - rightAmount - skipAmount}/{skipAmount}
                </td>
                <td>{life}</td>
                <td>{score}</td>
              </tr>
      ...
      {showPrinciple ? (
        <section className={style.principle}>
          <div className={style.qrCodeWrapper}>
            <img src={require('./asset/qrCode.png')} />
            <p>
              长按识别二维码并关注公众号 可获知实验原理--<em>公地悲剧</em>
            </p>
          </div>
          <img className={style.btnClose} src={require('./asset/close.png')} onClick={() => setShowPrinciple(false)} />
        </section>
      ) : null}
    </section>
  )
