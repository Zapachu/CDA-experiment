...
> {
    ...
    switch (type) {
      ...
      case MoveType.shout: {
        ...
        if (
          ...
          playerStateArray.every(ps => ps.answers && ps.answers.length === ITEMS.length)
        ) {
  ...
  private processGameState(gameState: IGameState, playerStates: Array<TPlayerState<IPlayerState>>) {
    const { groupSize } = this.game.params
    const totalScore = playerStates.reduce((acc, current) => acc + current.contribution, 0)
  ...
  private async nextItem(playerState: TPlayerState<IPlayerState>, answer: GARBAGE, index: number) {
    setTimeout(async () => {
      playerState.answers[index] = answer
      playerState.flyTo = undefined
