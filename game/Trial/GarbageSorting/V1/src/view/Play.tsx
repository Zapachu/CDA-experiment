...
const IMG_BIN = {
  [GARBAGE.kitchen]: require('./components/bin_kitchen.png'),
  [GARBAGE.other]: require('./components/bin_other.png'),
  [GARBAGE.recyclable]: require('./components/bin_recyclable.png'),
  [GARBAGE.hazardous]: require('./components/bin_hazardous.png')
}

const IMG_ITEM = {
  [ITEM_NAME.battery]: require('./asset/battery.svg'),
  [ITEM_NAME.bone]: require('./asset/bone.svg'),
  [ITEM_NAME.bottle]: require('./asset/bottle.svg'),
  [ITEM_NAME.bulb]: require('./asset/bulb.svg'),
  [ITEM_NAME.clothing]: require('./asset/clothing.svg'),
  [ITEM_NAME.earthenwareBowl]: require('./asset/earthenwareBowl.svg'),
  [ITEM_NAME.mud]: require('./asset/mud.svg'),
  [ITEM_NAME.popCan]: require('./asset/popCan.svg'),
  [ITEM_NAME.tissue]: require('./asset/tissue.svg'),
  [ITEM_NAME.vegetableLeaf]: require('./asset/vegetableLeaf.svg')
}

const defaultBtnStatus = {
  [GARBAGE.pass]: BTN.pass,
  [GARBAGE.kitchen]: BTN.default,
  [GARBAGE.other]: BTN.default,
...
const translateOffset = {
  ...
  [GARBAGE.kitchen]: { x: '-50%', y: '350%' },
  [GARBAGE.other]: { x: '150%', y: '350%' },
...
> {
  ...
  constructor(
    props: Core.IPlayProps<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams>
  ) {
    super(props as any)
    this.state = {
      shoutTimer: 0,
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.playerState.answers && prevProps.playerState.answers.length !== answers.length) {
      this.setState({
        btnStatus: { ...defaultBtnStatus },
  ...
  choose = (garbage: GARBAGE) => {
    ...
    this.setState(
      ...
      () => {
        frameEmitter.emit(MoveType.check, { answer: garbage, index: answers.length }, (err, isTrue: boolean) => {
          if (err) {
            return Toast.warn(err)
          }
          setTimeout(() => {
            if (garbage === GARBAGE.pass) {
              frameEmitter.emit(MoveType.shout, { answer: garbage, index: answers.length }, err => {
                Toast.warn(err)
              })
            } else {
              const btnStatus = { ...this.state.btnStatus }
    ...
    switch (status) {
      ...
      case STATUS.playing: {
        return (
          <div className={style.playing}>
            {this.renderPlaying()}
            <Modal visible={modal === MODAL.rule}>{this.renderRuleModal()}</Modal>
          </div>
        )
      }
      case STATUS.waiting: {
      ...
      case STATUS.result: {
        return (
          <div className={style.resultWrapper}>
            {this.renderResult()}
            <Modal visible={modal === MODAL.share}>{this.renderShareModal()}</Modal>
          </div>
        )
      }
    }
  ...
  renderPlaying = () => {
    ...
    return (
      <>
        <img className={style.rule} src={IMG_RULE} onClick={() => this.setState({ modal: MODAL.rule })} />
        ...
        <div className={style.itemContainer}>
          <img style={itemStyle} className={style.item} src={IMG_ITEM[item.img]} />
          <div className={style.gaugeContainer}>
            <CircleProgress ratio={(shoutTimer === SHOUT_TIMER ? 0 : shoutTimer) / (SHOUT_TIMER - 1)} />
          </div>
        </div>
        <div className={style.binsContainer}>
          {[GARBAGE.recyclable, GARBAGE.kitchen, GARBAGE.hazardous, GARBAGE.other].map(bin => {
            return (
            ...
            {[GARBAGE.recyclable, GARBAGE.kitchen, GARBAGE.hazardous, GARBAGE.other].map(bin => {
              return (
                <li key={bin} className={style.btn}>
                  <Btn className={style.circle} status={btnStatus[bin]} onClick={() => this.choose(bin)} />
                </li>
              )
            })}
          </ul>
        ...
        <div className={style.passContainer}>
          <Btn className={style.btn} status={btnStatus[GARBAGE.pass]} onClick={() => this.choose(GARBAGE.pass)} />
  ...
  renderPlayerRank = (rank: number) => {
    ...
    if (rank === 1) {
      return <img className={style.rank} src={require('./components/medal_gold.png')} />
    }
    if (rank === 2) {
      return <img className={style.rank} src={require('./components/medal_silver.png')} />
    }
    if (rank === 3) {
      return <img className={style.rank} src={require('./components/medal_copper.png')} />
    }
  }

  getTitleInfo = (score: number): { image: string; label: string } => {
    if (score > (TOTAL_SCORE / 3) * 2) {
      return { image: require('./components/env_good.png'), label: '环境健康' }
    }
    if (score < TOTAL_SCORE / 3) {
      return { image: require('./components/env_bad.png'), label: '环境污染' }
    }
    return { image: require('./components/env_mid.png'), label: '环境一般' }
  }
  ...
  renderResult = (resultStyle = {}) => {
    ...
    return (
      <div style={resultStyle}>
        <div className={style.result} style={{ backgroundImage: `url(${image})` }}>
                ...
                sortedPlayers.map(({ img }, i) => {
                  return (
                    ...
                    >
                      ...
                      <img className={style.header} src={img || DEFAULT_AVATAR} />
                    </li>
                  )
                })}
            </ul>
        ...
        <div className={style.shareButton} onClick={() => this.setState({ modal: MODAL.share })}>
          <img src={require('./components/share.png')} />
          <span>分享</span>
        </div>
      </div>
    )
    ...
    return (
      <div className={style.ruleModal}>
        <div className={style.ruleContent}>
          <img src={require('./components/rule_modal.png')} />
        ...
        <img src={IMG_CLOSE} onClick={() => this.setState({ modal: undefined })} />
      </div>
    )
  }
  ...
  renderShareModal = () => {
    ...
    return (
      <div className={style.shareModal}>
        <p>扫码分享到微信</p>
        <QR size={150} value={this.shareUrl + `?userId=${playerState.user.id}`} />
        <img src={IMG_CLOSE} onClick={() => this.setState({ modal: undefined })} />
      </div>
    )
  }
            ...
            <div className={style.binsContainer}>
              ...
              <div className={style.btnsContainer}>
                {[GARBAGE.recyclable, GARBAGE.kitchen, GARBAGE.hazardous, GARBAGE.other].map(bin => {
                  return (
                    <li key={bin} className={style.btn}>
                      <Btn className={style.btnCircle + ' ' + style.placeholder} status={BTN.default} />
...
async function initWechat(shareUrl: string) {
  ...
  const res = await ajax('/wechat/jssdk?url=' + encodeURIComponent(window.location.href))
  if (res.err) {
    return alert('微信加载出错')
  }
...
async function ajax(urlOrOptions: string | { url: string; method: string; data: object }): Promise<any> {
  if (typeof urlOrOptions === 'string' || urlOrOptions.method.toLowerCase() === 'get') {
