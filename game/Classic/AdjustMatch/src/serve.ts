...
const router = Router().get(FetchRoute.exportXls, async (req, res) => {
  ...
  rounds.forEach(round => {
    ...
    round.data.forEach(({ user, playerIndex, privatePrices, initGood, initGoodPrice, join, sort, good, goodPrice }) =>
      data.push([user, playerIndex, privatePrices, initGood, initGoodPrice, join, sort, good, goodPrice])
    )
  })
