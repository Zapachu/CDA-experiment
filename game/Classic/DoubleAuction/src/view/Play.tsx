...
function RoundPlay({playerRoundState, gameRoundState, frameEmitter, role, playerIndex}: {
    ...
    switch (status) {
        ...
        case PlayerRoundStatus.result:
            const {shouts} = gameRoundState, myShout = shouts[playerIndex];
            const traded = myShout && myShout.tradePair !== null;
            return <section className={style.roundResult}>
                <p>{lang.roundOver}{traded ? <>{lang.tradeInfo1}<em>{shouts[playerIndex].price}</em>&nbsp;,&nbsp;{lang.tradeInfo2}<em>{privatePrice}</em>&nbsp;,&nbsp;{lang.tradeInfo3}<em>{playerRoundState.profit}</em></> : lang.noTrade}</p>
            </section>;
    }
}
    ...
    render(): React.ReactNode {
        ...
        if (playerState.status === PlayerStatus.guide) {
            return <section className={style.groupGuide}>
                <p>本实验共进行R轮，每组N人。参与者会被随机分配到某一组中，担任卖家或者买家其中一种角色，实验中角色不变，双方人数对半，每轮进行1单位商品的交易；每轮实验中，每单位商品对于买家有一个货币价值，该价值是V1到V2之间的随机数，买家根据自身的货币价值进行出价（Bid
                    prices），买家的出价只有低于货币价值时才会获得收益；每单位商品对于卖家有一定的成本，该成本是C1到C2之间的随机数，卖家根据商品的成本进行要价（Ask
                    prices），卖家的要价只有高于商品成本时才能获得收益。卖家接受买家的出价或者买家同意卖家的要价时，交易达成。实验中可及时修改价格（降低要价、提高出价），最终收益计算：买家（Buyer）——每买进一单位商品，收益=货币价值-成交价格；卖家（Seller）——每卖出一单位商品，收益=成交价格-商品成本</p>
