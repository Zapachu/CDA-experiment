...
const router = Router()
    .get(FetchRoute.exportXls, async (req, res) => {
        ...
        rounds.forEach(round => {
            ...
            round.data.filter(s => s).forEach((shout: IShout, i) =>
                data.push([i + 1, shout.role === Role.buyer ? 'Buyer' : 'Seller', shout.price, !!shout.tradePair, shout.tradePair ? shout.tradePair + 1 : ''])
            );
        });
