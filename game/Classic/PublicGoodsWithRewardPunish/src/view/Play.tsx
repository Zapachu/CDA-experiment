                             ...
                             max={groupParams.M}/>
                {
                    groupParams.mode === Mode.normal ? null :
                        <>
                            ...
                            <InputNumber placeholder={`0≤d≤${groupParams.M - x}`} value={d} onChange={v => setD(+v)}
                                         min={0}
                                         max={groupParams.M - x}/>
                        </>
                }
                <br/>
                <Button type='primary' onClick={() => frameEmitter.emit(MoveType.submit, {x, d})}>{lang.submit}</Button>
            </section>;
    ...
    render(): React.ReactNode {
        ...
        if (playerState.status === PlayerStatus.guide) {
            return <section className={style.groupGuide}>
                <p>本实验共R轮。在本次实验中，您将会被随机的分配到某个组中，每组有N名成员。每名成员在实验开始时都有M实验币，您可以选择拿出一部分实验币出来作为公共资金投资。每组的公共资金总和将会翻K倍，然后平均返回给该组的N人。您的收益将等于您从您所在组的公共资金中获得的回报加上您未投入公共资金池的实验币。此外，可以选择花费一定的成本惩罚投资公共资金池最少的人，具体的惩罚机制为：花费惩罚成本的参与者在原来的基础上减少d，收到惩罚的参与者实验收益减少P*d，其中P为惩罚参数，大于0小于等于1。</p>
