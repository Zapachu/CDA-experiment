...
> {
  ...
  protected async playerMoveReducer(
    ...
    const {
      game: {
        params: { group, groupSize, round, groupParams }
      }
    } = this
    ...
    switch (type) {
      case MoveType.getPosition: {
        ...
        playerState.rounds = Array(round)
          .fill(null)
          .map((_, i) => ({
      ...
      case MoveType.submit: {
        ...
        if (playerStatus.every(status => status === PlayerStatus.submitted)) {
          setTimeout(async () => {
            ...
            groupRoundState.returnMoney = ~~(
              (groupPlayerStates.map(({ rounds }) => rounds[roundIndex].submitMoney).reduce((m, n) => m + n, 0) /
                groupPlayerStates.length) *
              groupParams[groupIndex].roundParams[roundIndex].K
            )
