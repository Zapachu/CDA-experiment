...
export function Play4Owner({
  game: {},
  playerStates,
  gameState: { logs, groups }
}: Core.IPlay4OwnerProps<ICreateParams, IGameState, IPlayerState, MoveType, PushType, IMoveParams, IPushParams>) {
      ...
      <div className={style.tabsWrapper}>
        <Tabs defaultActiveKey={activeTabKey} onChange={setActiveTabKey}>
          <Tabs.TabPane tab={lang.members} key={lang.members}>
            <Table
              dataSource={Object.entries(playerStates).map(([token, { groupIndex, user }]) => ({
                key: token,
                name: user.name,
                mobile: user.stuNum,
                group: groupIndex + 1,
          ...
          <Tabs.TabPane tab={lang.logs} key={lang.logs}>
            <Table
              dataSource={logs.reverse().map(([group, round, position, money, time]) => ({
                key: time,
                group: group + 1,
                round: round + 1,
