...
import { Affix, Button, Modal, Table } from 'antd'
        ...
        return (
          <div className={style.result}>
            ...
            {roundIndex === round - 1 ? (
              ...
              <p className={style.roundOverInfo}>
                {lang.toNewRound1}
                <em>{NEW_ROUND_TIMER - (newRoundTimers[roundIndex] || 0)}</em>
                {lang.toNewRound2}
              </p>
            )}
          </div>
  ...
  return (
    <section className={style.play}>
      <section className={style.gameDesc}>
        ...
        <p>
          {
            '本实验是关于公共物品的实验，实验分为G组，每组N人，共进行R轮，每位参与者将会被随机的分配到某个组中。在本次实验中，每位参与者有M实验币的初始禀赋，每轮实验参与者均可以选择拿出一部分实验币x出来作为该组公共资金投资。每组的公共资金总和将会翻K倍，然后平均返还给该组的N人。'
          }
        </p>
      </section>
      <p className={style.roundInfo}>
        {lang.roundInfo1}
        <em>{roundIndex + 1}</em>
        {lang.roundInfo2}
        <em>{round}</em>
      ...
      <Modal visible={showHistory} onOk={() => setShowHistory(false)} onCancel={() => setShowHistory(false)}>
        <Table
          dataSource={playerState.rounds.map(({ initialMoney, submitMoney }, i) => ({
            key: i,
            round: i + 1,
            initialMoney,
            submitMoney,
