...
const router = Router().get(FetchRoute.exportXls, async (req, res) => {
  ...
  const data = [],
    option = {}
  data.push(['玩家', '学号', '编号', '心理价值', '初始物品', '初始物品价格', '最终物品编号', '最终物品价格'])
  ...
  rounds.forEach(round => {
    ...
    round.data.forEach(({ userName, stuNum, indexInGroup, privatePrices, initGood, initGoodPrice, good, goodPrice }) =>
      data.push([userName, stuNum, indexInGroup, privatePrices, initGood, initGoodPrice, good, goodPrice])
    )
  })
