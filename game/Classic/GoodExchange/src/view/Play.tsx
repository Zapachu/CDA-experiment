...
function RoundPlay({playerRoundState, gameRoundState, frameEmitter, playerIndex}: {
    ...
    const {allocation, exchangeMatrix, timeLeft} = gameRoundState, {privatePrices} = playerRoundState;
    return <>
            ...
            <Table
                ...
                columns={[
                    ...
                    },
                    {
                        ...
                        dataIndex: 'good',
                        key: 'good',
                        render: good => <div style={colStyle}>
                            {
                                (() => {
                                    if (good === playerIndex) {
                                    ...
                                    if (exchangeMatrix[good][playerIndex] === ExchangeStatus.waiting) {
                                        ...
                                        return <Button {...btnProps} type={'primary'}>{lang.reqExchange}</Button>;
                                    }
                                })()
                            }
                        </div>
        ...
        <div className={style.roundResult}>
            {
                allocation[playerIndex] === null ? null : <>
                    <p className={style.resultInfo}>
                        {lang.resultInfo1}<em>{playerIndex + 1}</em>&nbsp;,&nbsp;
                        {lang.resultInfo2}<em>{privatePrices[playerIndex]}</em>&nbsp;,&nbsp;
                        {lang.resultInfo3}<em>{allocation[playerIndex] + 1}</em>&nbsp;,&nbsp;
                        {lang.resultInfo4}<em>{privatePrices[allocation[playerIndex]]}</em>
                    </p>
