...
const router = Router().get(FetchRoute.exportXls, async (req, res) => {
  ...
  const data = [],
    option = {}
  data.push(['玩家', '学号', '编号', '优先序', '心理价值', '偏好表达', '最终物品', '最终物品价格'])
  ...
  rounds.forEach(round => {
    ...
    round.data.forEach(({ userName, stuNum, indexInGroup, indexInRound, privatePrices, sort, good, goodPrice }) =>
      data.push([userName, stuNum, indexInGroup, indexInRound, privatePrices, sort, good, goodPrice])
    )
  })
