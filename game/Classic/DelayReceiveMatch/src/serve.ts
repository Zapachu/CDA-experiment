...
const router = Router().get(FetchRoute.exportXls, async (req, res) => {
  ...
  const data = [],
    option = {}
  data.push(['玩家', '优先序', '心理价值', '偏好表达', '分得物品编号', '分得物品价格'])
  ...
  rounds.forEach(round => {
    ...
    round.data.forEach(({ user, playerIndex, privatePrices, sort, good, goodPrice }) =>
      data.push([user, playerIndex, privatePrices, sort, good, goodPrice])
    )
  })
