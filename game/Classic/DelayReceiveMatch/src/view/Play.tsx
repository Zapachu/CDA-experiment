...
function RoundPlay({groupParams, playerRoundState, gameRoundState, frameEmitter, playerIndex}: {
    ...
    const {allocation} = gameRoundState, {privatePrices, status} = playerRoundState;
    const [sort, setSort] = React.useState(Array(groupParams.goodAmount).fill(null).map((_, i) => i));
    switch (status) {
