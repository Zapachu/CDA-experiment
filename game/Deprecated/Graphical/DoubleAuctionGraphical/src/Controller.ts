...
> {
  ...
  protected async playerMoveReducer(
    ...
    const {
      game: {
        params: { groupSize, round, positions, countdown }
      }
    } = this
    ...
    switch (type) {
      ...
      case MoveType.shout: {
        ...
        if (!rounds[roundIndex].hasTimer) {
          ...
          const countdownInterval = global.setInterval(async () => {
            groupPlayerStates.forEach(({ actor }) =>
              this.push(actor, PushType.countdown, {
                roundIndex,
                countdown: timeAcc
              })
            )
      ...
      case MoveType.deal: {
        ...
        groupPlayerStates[params.position].profits[roundIndex] =
          groupPlayerStates[params.position].privatePrices[roundIndex] - playerState.prices[roundIndex]
