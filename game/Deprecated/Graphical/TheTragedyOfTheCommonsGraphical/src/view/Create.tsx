...
export class Create extends Core.Create<ICreateParams, ICreateState> {
  ...
  componentDidMount(): void {
    ...
    setParams({
      ...
      groupSize,
      fishCount,
      magnification: parseFloat(magnification)
    })
  }
    ...
    return (
      <div className={style.create}>
        <ul className={style.configFields}>
          ...
          <li>
            <Label label="初始鱼的条数" />
            <RangeInput value={fishCount} onChange={e => this.setState({ fishCount: parseInt(e.target.value) })} />
          </li>
          <li>
            <Label label="翻倍比率" />
            <RangeInput value={magnification} onChange={e => this.setState({ magnification: e.target.value })} />
          </li>
        </ul>
