...
import { Button, Input, Label, Lang, MaskLoading, RangeInput, Toast } from '@elf/component'
...
export class Create extends Core.Create<ICreateParams, ICreateState> {
  ...
  genePrivatePrice({ round, groupSize, startingPrice }: ICreateParams, privatePriceLimit: number) {
    return Array(round)
      ...
      .map(() =>
        Array(groupSize)
          .fill(null)
          .map(() => ~~(Math.random() * (privatePriceLimit - startingPrice)) + startingPrice)
          .join(',')
      )
  }
  ...
  edit() {
    ...
    const {
      ...
      props: {
        setParams,
        setSubmitable,
        params: { round, groupSize, startingPrice, privatePrice }
      }
    } = this
    const privatePriceTable = Array(round)
      .fill(null)
      .map((_, r) =>
        Array(groupSize)
          .fill(null)
          .map((_, c) => (privatePrice[r] || '').split(',')[c])
      )
    ...
    setParams({
      ...
      groupSize,
      startingPrice,
      privatePrice: privatePriceTable.map(arr => arr.join(','))
    })
    this.setState({
      readonly: true
    })
    ...
    return (
      <section className={style.create}>
        ...
        <div className={style.btnSwitch}>
          {readonly ? <a onClick={() => this.edit()}>{lang.edit}</a> : <a onClick={() => this.done()}>{lang.done}</a>}
        </div>
      </section>
    )
  }
  ...
  renderBaseFields() {
    const {
      lang,
      props: {
        params: { round, groupSize, startingPrice },
        setParams
      },
  ...
  renderPrivatePrice() {
    const {
      lang,
      props: {
        params: { round, groupSize, privatePrice },
        setParams
      },
      state: { readonly }
    } = this
              ...
              <tr key={r}>
                ...
                {Array(groupSize)
                  ...
                  .map((_, c) => {
                    ...
                    return (
                      <td key={c} className={readonly ? style.readonly : null}>
                        <Input
                          value={value}
                          onChange={({ target: { value } }) => {
                            if (readonly) {
