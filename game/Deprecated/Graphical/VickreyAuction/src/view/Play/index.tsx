...
function Player({
  ...
  const { opacity } = useSpring({
    opacity: playerStatus == PlayerStatus.outside ? 0 : 1
  })
  return (
    <ImgLoader
      src={[
      ...
      render={({ images: [playerL, playerR, playerL_dealed, playerR_dealed] }) => {
        const [player_normal, player_dealed] =
            direction === Direction.L ? [playerR, playerR_dealed] : [playerL, playerL_dealed],
          player = playerStatus == PlayerStatus.won ? player_dealed : player_normal
        return (
          <animated.g opacity={opacity}>
            {playerStatus == PlayerStatus.won ? (
              <g transform={`translate(${span(-0.2)},${span(-0.6)})`}>
                <Win />
              </g>
            ) : null}
            <g transform={`translate(${player.width >> 1},${player.height * 0.9})`}>
            ...
            />
            {privatePrice && playerStatus == PlayerStatus.prepared ? <Idea msg={privatePrice} /> : null}
          </animated.g>
        )
      }}
    />
...
function Envelope({ playerStatus }: { playerStatus: PlayerStatus }) {
  switch (playerStatus) {
    case PlayerStatus.prepared:
      return <image href={require('./img/envelope_open.svg')} width={span(0.8)} />
    case PlayerStatus.shouted:
      const { x, y, width } = useSpring({
        x: -2,
        y: -5,
        width: 0,
        from: { x: 0, y: 0, width: 0.8 },
        delay: 1000,
        config: {
          duration: 800
        }
...
> {
        ...
        <Stage>
          {newRoundTimer ? (
            ...
            <>
              ...
              <g transform={`translate(${span(5)},${span(0.5)})`}>
                <Paint index={roundIndex} />
              </g>
              <g transform={`translate(${span(1)},${span(1.8)})`}>
                <Host msg={someoneWon ? lang.tradeSuccess : lang.curPaintStartingPrice(startingPrice)} />
              </g>
              {this.renderPlayers()}
  ...
  renderPlayers() {
    ...
    return (
      <g transform={`translate(${span(1.8)},${span(4.5)})`}>
        {PlayersCfg.map(({ direction, role }, i) => (
          <g
            key={i}
            transform={`translate(${span(1.8 * i)},0)`}
            opacity={role === Role.player ? 1 : role === Role.partner ? 0.7 : 0.4}
          >
            <Player
              key={i}
              playerStatus={
