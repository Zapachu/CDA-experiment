...
> {
  lang = Lang.extractLang({
    ...
    instruction: [
      '伯川德竞争：每位成员代表一家公司报价，买方市场总是购买价格更低的同类产品，因此出价更高的公司会被淘汰',
      'Betrand Competition: every participant represents their company to set a price, the buyer market always purchase the one at lower price, so companies that set higher prices will fail'
    ],
    prepare: ['接下来请各位参与者开始报价吧', 'Please set a price'],
    conclusion: [
      (n, m) => `恭喜${n}号，成功卖出产品，最后的成交价格为${m}`,
      (n, m) => `Congratulations to number ${n}, you have successfully sold out your product, the strike price was ${m}`
    ],
    next: ['下一轮', 'Next round']
  })
          ...
          {newRoundTimer ? (
            ...
            <>
              <g transform={`translate(${span(2.4)},${span(1.4)})`}>
                <Host widthScale={1.4} msg={this.renderHostMessage()} />
              </g>
              <Players
