          ...
          <g opacity={num !== undefined ? 1 : 0.7}>
            ...
            {num !== undefined ? (
              direction === Direction.B ? (
                <g fontSize={'20px'} transform={`translate(${player.width * -0.2},${player.height * 1.2})`}>
                  <text>
                    上轮成绩为
                ...
                <g fontSize={'20px'} transform={`translate(${player.width * -0.1},${player.height * 1.1})`}>
                  <Bucket num={num} />
                </g>
              )
            ) : null}
...
const Bucket = ({ num }) => {
  ...
  return (
    <>
      ...
      <text transform={`translate(${span(0.12)},${span(0.65)})`}>
        鱼量:
        <tspan fill={'#ffd466'}>{num}</tspan>
      </text>
    </>
