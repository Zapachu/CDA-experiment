...
import { span, Stage, Host, Button, Input, RoundSwitching } from '@bespoke-game/graphical-util'
...
> {
  lang = Lang.extractLang({
    ...
    instruction: [
      '古诺竞争：两位参与成员，您选择的产量越高，市场价格越低，您选择的产量越低，市场价格越高',
      'Cournot Competition: two participants, the higher you choose, the lower the price will be; the lower you choose, the higher the price will be'
    ],
    prepare: [
      n =>
        `当前鱼塘中鱼的数量为${n}，你最多可捕鱼${Math.floor(
          n / 2
        )}条，捕鱼数量越多，则价格越低；捕鱼数量越少，则价格越高`,
      n =>
        `The number of fish in the fishpond currently is ${n}，you can catch at most ${Math.floor(
          n / 2
        )} fish，the more fish you catch, the lower the price will be; the less fish you catch, the higher the price will be`
    ],
    conclusion: [
      (n, m) => `两边捕鱼完成，您的当前捕鱼数量为${n}，则剩下的产量为${m}，即每条鱼的价格为${m}`,
      (n, m) =>
        `Fishing is over，you caught ${n} fish while ${m} fish are left in the fishpond，which means that price of each fish is ${m}`
    ],
          ...
          {newRoundTimer ? (
            ...
            <>
              <g transform={`translate(${span(2.4)},${span(1.4)})`}>
                <Host widthScale={1.3} msg={this.renderHostMessage()} />
              </g>
              <Players
                playerStatus={curPlayerStatus}
                lastProfit={roundIndex > 0 ? profits[roundIndex - 1] : 0}
                curQuantity={quantities[roundIndex] || 0}
              />
        ...
        return (
          <>
            <text fontSize={'20px'} transform={`translate(${span(3.5)},${span(8)})`}>
              ...
              <tspan fontSize={'24px'} fill={'#ffd466'}>
                {profits[roundIndex]}
              </tspan>
            </text>
            <foreignObject
