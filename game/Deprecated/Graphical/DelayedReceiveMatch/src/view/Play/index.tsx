      ...
      render={({
        ...
        return (
          <g>
            ...
            {showBack ? (
              <image href={playerBack.src} x={-playerBack.width >> 1} y={span(0.5)} />
            ) : (
              <>
                <image href={body.src} x={-body.width >> 1} y={span(0.75)} />
                <image href={face.src} x={-face.width >> 1} y={span(0.15)} />
                <image href={hair.src} x={-hair.width >> 1} />
                {matched ? (
                  <g transform={`translate(0,${span(1.5)})`}>
                    <Good {...matched} />
                  </g>
                ) : null}
              </>
...
function Good({ index, price, scale = 1 }: { index: number; price: number; scale?: number }) {
  ...
  const [{ offsetX, offsetY }, setOffsetXY] = React.useState({
    offsetX: 0,
    offsetY: 0
  })
    ...
    <ImgLoader
      ...
      render={({ images: [box] }) => (
        ...
        >
          <image href={box.src} />
          <g
            transform={`translate(${box.width >> 1},${span(0.5)})`}
            style={{
              userSelect: 'none',
              textAnchor: 'middle',
              fontSize: 18
...
> {
        ...
        <Stage>
          {newRoundTimer ? (
            ...
            <>
              ...
              <g transform={`translate(0,${span(8)})`}>{this.renderOperateWidget()}</g>
            </>
          )}
        </Stage>
  ...
  renderPlay() {
    return (
      <g transform={`translate(${span(1.2)},${span(3.5)})`}>
    ...
    return (
      <g transform={`translate(${span(1.2)},${span(3.5)})`}>
        {Array(groupSize)
          ...
          .map(({ i, j }) => (
            <g
              transform={`translate(${span(2.1 * (j % ROW_SIZE) + ~~(j / ROW_SIZE))},${span(~~(j / ROW_SIZE) * 1.5)})`}
            >
              <Player
                key={j}
                active={i === positionIndex}
                showBack={showBack}
  ...
  renderOperateWidget() {
    ...
    switch (playerStatus[positionIndex]) {
      case PlayerStatus.outside: {
        return (
          <g transform={`translate(${span(5)},${span(1)})`}>
      ...
      case PlayerStatus.prepared: {
        return (
          <g transform={`translate(${span(4)},${span(1)})`}>
...
namespace Preference {
  ...
  export function ByDrag(props: IPreferenceProps) {
    const initialItems = props.privatePrices.map((price, i) => ({
      id: i.toString(),
    ...
    function handleDragEnd({ source, destination }: { source: IDragTarget; destination?: IDragTarget }) {
  ...
  const itemStyle: React.CSSProperties = {
    ...
    height: spanDom(0.8),
    marginTop: spanDom(0.1),
    display: 'flex',
    justifyContent: 'center',
    alignItems: 'center',
    fontSize: spanDom(0.35)
  }
