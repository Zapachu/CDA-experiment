...
import { Lang, Label, RangeInput, Button, MaskLoading, Input, Toast, Tabs } from '@elf/component'
...
export class Create extends Core.Create<ICreateParams, ICreateState> {
  lang = Lang.extractLang({
    ...
    checkPrivatePricePls: [
      ...
      (g, r, p, good) =>
        `Private prices of players are invalid, check them please : Group${g + 1}:Round${r + 1}:Player${p +
          1}:Good${good + 1}`
    ]
  })
  state: ICreateState = {
  ...
  edit() {
    ...
    const {
      ...
      props: {
        setSubmitable,
        params: { round, groupSize, group, groupPPs },
        setParams
      }
    } = this
    let invalidIndex: {
    ...
    const _groupPPs = Array(group)
      .fill(null)
      .map((_, groupIndex) => ({
        roundPPs: Array(round)
          .fill(null)
          .map((_, roundIndex) => ({
            playerPPs: Array(groupSize)
              .fill(null)
              .map((_, playerIndex) => ({
                privatePrices: Array(groupSize)
                  .fill(null)
                  .map((_, goodIndex) => {
                    if (invalidIndex) {
                      return null
                    }
                    const price = +groupPPs[groupIndex].roundPPs[roundIndex].playerPPs[playerIndex].privatePrices[
                      goodIndex
                    ]
  ...
  geneGroupPrivatePrices({ group, groupSize, round }: ICreateParams): Array<CreateParams.IGroupPP> {
    ...
    return Array(RANGE.group.max)
      .fill(null)
      .map((_, groupIndex) => ({
        roundPPs: Array(RANGE.round.max)
          .fill(null)
          .map((_, roundIndex) => ({
            playerPPs: Array(RANGE.groupSize.max)
              .fill(null)
              .map((_, playerIndex) => ({
                privatePrices: Array(RANGE.groupSize.max)
                  .fill(null)
                  .map((_, goodIndex) =>
                    groupIndex < group && roundIndex < round && playerIndex < groupSize && goodIndex < groupSize
  ...
  renderBaseFields() {
    ...
    return (
      <ul className={style.configFields} style={{ visibility: submitable ? 'hidden' : 'visible' }}>
        <li>
        ...
        <Button label={lang.generate} onClick={async () => await this.updatePrivatePrice()} />
      </ul>
    )
  }
  ...
  render(): React.ReactNode {
    ...
    return (
      <section className={style.create}>
        ...
        <Tabs
          labels={groupIterator.map((_, i) => lang.groupIndex(i))}
        ...
        >
          {groupIterator.map((_, i) => (
            <Tabs
              key={i}
              labels={roundIterator.map((_, i) => lang.roundIndex(i))}
              ...
              <tr key={r}>
                ...
                {Array(groupSize)
                  ...
                  .map((_, c) => {
                    ...
                    return (
                      <td key={c} className={submitable ? style.readonly : null}>
                        <Input
                          value={value}
                          onChange={({ target: { value } }) => {
                            if (submitable) {
