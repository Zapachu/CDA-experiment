...
const winnerModes = ['最高出价的前 m 位玩家胜利', '出价大于所有玩家出价中位数的 m位玩家胜利']
    ...
    return (
      <div className={style.create}>
        <ul className={style.configFields}>
          ...
          <li>
            <Label label="买家公共价值" />
            <RangeInput value={commonValue} onChange={e => this.setState({ commonValue: parseInt(e.target.value) })} />
          </li>
          <li>
            <Label label="买家价值偏差" />
            <RangeInput value={deviation} onChange={e => this.setState({ deviation: parseInt(e.target.value) })} />
          </li>
          <li>
            <Label label="获胜方式" />
            <RadioGroup value={mode} options={winnerModes} onChange={v => this.setState({ mode: v })} />
          </li>
          <li>
