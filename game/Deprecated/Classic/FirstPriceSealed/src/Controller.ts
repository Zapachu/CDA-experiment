...
> {
  ...
  protected async playerMoveReducer(
    ...
    const {
      game: {
        params: { groupSize, round, positions }
      }
    } = this
    ...
    switch (type) {
      ...
      case MoveType.shout: {
        ...
        if (playerStatus.every(status => status === PlayerStatus.shouted)) {
          ...
          groupPlayerStates[0].profits[roundIndex] =
            groupPlayerStates[0].privatePrices[roundIndex] - groupPlayerStates[1].prices[roundIndex]
